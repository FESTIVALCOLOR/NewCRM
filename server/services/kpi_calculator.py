...
from sqlalchemy import Date, DateTime, and_, cast, extract, func, or_
...
def _get_period_range(year: int, quarter: Optional[int] = None, month: Optional[int] = None):
...
def _get_completed_stages(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> list:
...
def calc_k_deadline(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> float:
...
def calc_k_quality(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> float:
...
def calc_k_speed(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> float:
...
def calc_k_nps_crm(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> Optional[float]:
    ...
    surveys_direct = (
        db.query(ClientSurvey)
        .join(Contract, ClientSurvey.contract_id == Contract.id)
...
def calc_crm_kpi(db: Session, employee_id: int, position: str, project_type_db: str, period_start: date, period_end: date) -> dict:
    ...
    if k_nps is None:
        ...
        if remaining > 0:
            kpi_total = (weights["deadline"] / remaining) * k_deadline + (weights["quality"] / remaining) * k_quality + (weights["speed"] / remaining) * k_speed
        else:
            kpi_total = (k_deadline + k_quality + k_speed) / 3
    else:
        kpi_total = weights["deadline"] * k_deadline + weights["quality"] * k_quality + weights["speed"] * k_speed + weights["nps"] * k_nps

    # Дополнительные метрики
    stages = _get_completed_stages(db, employee_id, project_type_db, period_start, period_end)
    stages_with_dl = [s for s in stages if s.deadline and _parse_date(s.deadline)]

    on_time = sum(1 for s in stages_with_dl if _parse_date(s.completed_date) and _parse_date(s.deadline) and _parse_date(s.completed_date) <= _parse_date(s.deadline))
...
def calc_k_procurement(db: Session, employee_id: int, period_start: date, period_end: date) -> float:
...
def calc_k_defects(db: Session, employee_id: int, period_start: date, period_end: date) -> float:
...
def calc_k_nps_supervision(db: Session, employee_id: int, period_start: date, period_end: date) -> Optional[float]:
    """Knps для надзора — средний NPS по надзорным проектам."""
    surveys = (
        db.query(ClientSurvey)
        .join(Contract, ClientSurvey.contract_id == Contract.id)
        .join(SupervisionCard, SupervisionCard.contract_id == Contract.id)
        .filter(
...
def calc_supervision_kpi(db: Session, employee_id: int, position: str, period_start: date, period_end: date) -> dict:
...
def _calc_supervision_kpi_inner(db: Session, employee_id: int, position: str, period_start: date, period_end: date) -> dict:
    ...
    if card_ids:
        visits_count = (
            db.query(func.count(SupervisionVisit.id))
            .filter(
    ...
    if k_nps is None:
        remaining = 1.0 - weights["nps"]
        if remaining > 0:
            kpi_total = (weights["procurement"] / remaining) * k_procurement + (weights["defects"] / remaining) * k_defects + (weights["visits"] / remaining) * k_visits
        else:
            kpi_total = (k_procurement + k_defects + k_visits) / 3
    else:
        kpi_total = weights["procurement"] * k_procurement + weights["defects"] * k_defects + weights["visits"] * k_visits + weights["nps"] * k_nps
    ...
    procurement_overdue = sum(1 for e in procurement_entries if _parse_date(e.actual_date) and _parse_date(e.plan_date) and _parse_date(e.actual_date) > _parse_date(e.plan_date))

    active_cards = sum(1 for c in cards if c.column_name not in (STATUS_COMPLETED, STATUS_TERMINATED))
...
def calculate_employee_kpi(db: Session, employee_id: int, position: str, project_type: str, year: int, quarter: Optional[int] = None, month: Optional[int] = None) -> dict:
    """
    ...
    try:
        ...
        if project_type == "supervision":
            ...
            return calc_crm_kpi(db, employee_id, position, project_type_db, period_start, period_end)
    except Exception:
        logger.exception(
...
def get_concurrent_projects(db: Session, employee_id: int, project_type: str, position: str) -> int:
    """Возвращает текущее кол-во одновременных проектов/стадий."""
    if project_type == "supervision":
        return (
            db.query(func.count(SupervisionCard.id))
            .filter(
                or_(
                    SupervisionCard.dan_id == employee_id,
                    SupervisionCard.senior_manager_id == employee_id,
    ...
    if position == POSITION_SENIOR_MANAGER:
        return (
            db.query(func.count(CRMCard.id))
            .join(Contract, CRMCard.contract_id == Contract.id)
            .filter(
                Contract.project_type == project_type_db,
                CRMCard.senior_manager_id == employee_id,
    ...
    return (
        db.query(func.count(StageExecutor.id))
        .join(CRMCard, StageExecutor.crm_card_id == CRMCard.id)
        .join(Contract, CRMCard.contract_id == Contract.id)
        .filter(
            Contract.project_type == project_type_db,
...
def get_total_salary(db: Session, employee_id: int, project_type: str, period_start: date, period_end: date) -> float:
...
def get_total_area(db: Session, employee_id: int, project_type_db: str, period_start: date, period_end: date) -> float:
    ...
    result = db.query(func.coalesce(func.sum(Contract.area), 0)).filter(Contract.id.in_(contract_ids_q.subquery().select())).scalar()
