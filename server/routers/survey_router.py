...
YANDEX_FORM_IDS = {
    'individual': '',  # Заполняется при настройке
    'template': '',
    'supervision': '',
}
...
def _generate_survey_link(survey: ClientSurvey) -> str:
    ...
    return (
        f"https://forms.yandex.ru/u/{form_id}/"
...
def _apply_scores(survey: ClientSurvey, answers: dict):
    ...
    survey.nps_score = _safe_int(answers.get('q1_nps'))
    survey.csat_score = _safe_int(answers.get('q2_csat'))
    survey.design_score = _safe_int(answers.get('q3_design'))
    survey.deadline_score = _safe_int(answers.get('q4_deadline'))
    survey.communication_score = _safe_int(answers.get('q5_communication'))
    survey.expectations_score = _safe_int(answers.get('q6_expectations'))
    supervision = answers.get('q7_supervision')
...
async def handle_yandex_form_webhook(
    ...
    Формат данных зависит от настроек формы. Яндекс Формы (бизнес)
    отправляет JSON с полями формы.

    Защита: проверяем survey_token из скрытого поля формы.
    Повторные ответы с тем же токеном игнорируются.
    """
    try:
    ...
    # Яндекс Формы отправляет ответы в разных форматах.
    # Поддерживаем два варианта:
    # 1) {answers: [{question_id: "...", value: "..."}]}
    # 2) {field_name: value, ...}  (плоский формат)
    answers = {}

    if 'answers' in data and isinstance(data['answers'], list):
        # Формат массива ответов
        for item in data['answers']:
            qid = item.get('question_id', item.get('slug', ''))
            val = item.get('value', item.get('text', ''))
            answers[qid] = val
    else:
        # Плоский формат — ключи = ID вопросов
        answers = {k: v for k, v in data.items() if k != 'answer_id'}

    # Извлекаем токен
    token = (
        answers.get('hidden_survey_token')
        or answers.get('survey_token')
        or data.get('survey_token')
    )

    if not token:
        logger.warning("Webhook: отсутствует survey_token")
