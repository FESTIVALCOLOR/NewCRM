...
from constants import POSITION_STUDIO_DIRECTOR, ROLE_ADMIN, ROLE_DIRECTOR
...
def _check_webhook_rate_limit(client_ip: str) -> bool:
    ...
    if len(hits) >= WEBHOOK_RATE_LIMIT:
        ...
        logger.warning(f"Webhook rate limit: IP {client_ip} заблокирован на {WEBHOOK_BLOCK_DURATION}с ({len(hits)} запросов за {WEBHOOK_RATE_WINDOW}с)")
        return False
...
def _generate_survey_link(survey: ClientSurvey) -> str:
    ...
    return f"https://forms.yandex.ru/cloud/{form_id}/?contract_id={survey.contract_id}&project_type={survey.project_type}&survey_token={survey.access_token}"
...
def _is_admin(employee: Employee) -> bool:
    ...
    return role in (ROLE_ADMIN, ROLE_DIRECTOR) or position == POSITION_STUDIO_DIRECTOR
...
async def _try_send_survey_email(survey: ClientSurvey, contract: Contract, db: Session):
    """Отправляет email клиенту с ссылкой на опрос. Молча логирует ошибки."""
    if not contract or not contract.client_id:
        return
    client = db.query(Client).filter(Client.id == contract.client_id).first()
    if not client or not client.email:
        logger.info(f"Клиент без email, survey_id={survey.id} — email не отправлен")
        return
    try:
        email_svc = get_email_service()
        link = _generate_survey_link(survey)
        await email_svc.send_survey_email(
            to_email=client.email,
            client_name=client.full_name or "Клиент",
            contract_number=contract.contract_number or "",
            project_address=contract.address or "",
            survey_link=link,
        )
        logger.info(f"Email опроса отправлен на {client.email}, survey_id={survey.id}")
    except Exception as exc:
        logger.warning(f"Не удалось отправить email опроса survey_id={survey.id}: {exc}")
...
async def create_survey(
    ...
    logger.info(f"Опрос создан: survey_id={survey.id}, contract_id={req.contract_id}, type={req.project_type}, by={current_user.full_name}")

    await _try_send_survey_email(survey, contract, db)
...
async def resend_survey(
    ...
    contract = db.query(Contract).filter(Contract.id == survey.contract_id).first()
    await _try_send_survey_email(survey, contract, db)
...
async def handle_yandex_form_webhook(
    ...
    logger.info(f"Webhook: опрос завершён, survey_id={survey.id}, contract_id={survey.contract_id}, NPS={survey.nps_score}")
...
async def manual_import_survey(
    ...
    logger.info(f"Ручной импорт: survey_id={survey.id}, contract_id={survey.contract_id}, by={current_user.full_name}")
...
async def list_surveys(
    ...
    total = query.count()
    surveys = query.order_by(ClientSurvey.created_at.desc()).offset(offset).limit(limit).all()
