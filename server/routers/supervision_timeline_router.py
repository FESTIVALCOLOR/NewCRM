...
from database import Contract, Employee, SupervisionCard, SupervisionTimelineEntry, get_db
...
SUPERVISION_STAGES = [
    ...
    ("STAGE_CEILING", "Стадия 9: Закупка потолочных материалов"),
    ("STAGE_9_APPLIANCES", "Стадия 10: Бытовая техника"),
    ("STAGE_10_CUSTOM_FURNITURE", "Стадия 11: Закупка заказной мебели"),
    ("STAGE_11_FACTORY_FURNITURE", "Стадия 12: Закупка фабричной мебели"),
    ("STAGE_12_DECOR", "Стадия 13: Закупка декора"),
]


@router.get("/")
async def get_all_supervision_timelines(current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Получить все записи таймлайна надзора"""
    entries = db.query(SupervisionTimelineEntry).order_by(SupervisionTimelineEntry.supervision_card_id, SupervisionTimelineEntry.sort_order).all()
    return {"entries": [{c.name: getattr(e, c.name) for c in e.__table__.columns} for e in entries], "total": len(entries)}


@router.get("/{card_id}")
async def get_supervision_timeline(card_id: int, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Получить таблицу сроков надзора"""
    entries = db.query(SupervisionTimelineEntry).filter(SupervisionTimelineEntry.supervision_card_id == card_id).order_by(SupervisionTimelineEntry.sort_order).all()
    ...
    return {
        "entries": [{c.name: getattr(e, c.name) for c in e.__table__.columns} for e in entries],
        "totals": totals,
    }


@router.post("/{card_id}/init")
async def init_supervision_timeline(card_id: int, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    existing_entries = db.query(SupervisionTimelineEntry).filter(SupervisionTimelineEntry.supervision_card_id == card_id).order_by(SupervisionTimelineEntry.sort_order).all()

    if existing_entries:
        existing_codes = {e.stage_code for e in existing_entries}
        changed = False
        for i, (code, name) in enumerate(SUPERVISION_STAGES):
            if code not in existing_codes:
                # Сдвигаем sort_order всех записей начиная с позиции i+1
                ...
                # Обновляем имя если стадия была переименована
                entry = next((e for e in existing_entries if e.stage_code == code), None)
                if entry and entry.stage_name != name:
                    entry.stage_name = name
                    changed = True
        if changed:
            db.commit()
        return {"status": "already_initialized", "count": len(existing_entries)}
    ...
    for i, (code, name) in enumerate(SUPERVISION_STAGES):
        record = SupervisionTimelineEntry(supervision_card_id=card_id, stage_code=code, stage_name=name, sort_order=i + 1, status="Не начато", plan_date=plan_dates.get(i))
        db.add(record)
...
async def update_supervision_timeline_entry(card_id: int, stage_code: str, update: SupervisionTimelineUpdate, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Обновить запись таблицы сроков надзора"""
    entry = db.query(SupervisionTimelineEntry).filter(SupervisionTimelineEntry.supervision_card_id == card_id, SupervisionTimelineEntry.stage_code == stage_code).first()
    if not entry:
    ...
    if new_status and new_status != old_status and new_status.lower() in ("выполнено", "завершено"):
        asyncio.create_task(trigger_supervision_notification(card_id, "supervision_stage_complete", stage_name=entry.stage_name))

    return {c.name: getattr(entry, c.name) for c in entry.__table__.columns}


@router.get("/{card_id}/summary")
async def get_supervision_timeline_summary(card_id: int, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Сводка по таблице сроков надзора"""
    import json as _json

    from database import SupervisionBudgetExtra as _BudgetExtra

    entries = db.query(SupervisionTimelineEntry).filter(SupervisionTimelineEntry.supervision_card_id == card_id).all()
    ...
    # Добавляем строительные работы и черновые материалы
    extra = db.query(_BudgetExtra).filter(_BudgetExtra.supervision_card_id == card_id).first()
    if extra:
        total_budget_planned += (extra.construction_planned or 0) + (extra.materials_planned or 0)
        for p in _json.loads(extra.construction_payments or "[]"):
            total_budget_actual += float(p.get("amount", 0) or 0)
            total_commission += float(p.get("commission", 0) or 0)
        for p in _json.loads(extra.materials_payments or "[]"):
            total_budget_actual += float(p.get("amount", 0) or 0)
            total_commission += float(p.get("commission", 0) or 0)
...
async def export_supervision_timeline_excel(card_id: int, include_commission: bool = True, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if include_commission:
        headers = ["Стадия", "План. дата", "Факт. дата", "Дней", "Исполнитель", "Бюджет план", "Бюджет факт", "Экономия", "Поставщик", "Комиссия", "Статус", "Примечания"]
        widths = [30, 14, 14, 10, 18, 14, 14, 14, 18, 14, 14, 25]
    else:
        headers = ["Стадия", "План. дата", "Факт. дата", "Дней", "Исполнитель", "Бюджет план", "Бюджет факт", "Экономия", "Поставщик", "Статус", "Примечания"]
    ...
    if include_commission:
        total_row_data = ["ИТОГО:", "", "", "", "", total_planned, total_actual, total_savings, "", total_commission, "", ""]
    else:
        total_row_data = ["ИТОГО:", "", "", "", "", total_planned, total_actual, total_savings, "", "", ""]
...
async def export_supervision_timeline_pdf(card_id: int, include_commission: bool = False, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Экспорт таблицы сроков надзора в PDF (с/без комиссии, с ИТОГО)."""
    from pdf_helper import COLOR_GRANDTOTAL, _font, _font_bold, build_timeline_pdf
    ...
    if include_commission:
        headers = ["Стадия", "План. дата", "Факт. дата", "Дней", "Исполнитель", "Бюджет план", "Бюджет факт", "Поставщик", "Комиссия", "Статус", "Примечания"]
        col_widths = [120, 55, 55, 35, 80, 55, 55, 80, 50, 55, 80]
    else:
        headers = ["Стадия", "План. дата", "Факт. дата", "Дней", "Исполнитель", "Бюджет план", "Бюджет факт", "Поставщик", "Статус", "Примечания"]
