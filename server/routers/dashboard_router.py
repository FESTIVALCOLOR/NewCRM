...
@router.get("/clients")
async def get_clients_dashboard(year: Optional[int] = None, agent_type: Optional[str] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 2. Всего физлиц
        total_individual = db.query(Client).filter(Client.client_type == "Физическое лицо").count()

        # 3. Всего юрлиц
        total_legal = db.query(Client).filter(Client.client_type == "Юридическое лицо").count()
        ...
        if year:
            year_str = str(year)
            clients_by_year = db.query(Contract.client_id).filter(Contract.contract_date.like(f"{year_str}-%")).distinct().count()

        # 5. Клиенты агента (всего)
        agent_clients_total = 0
        if agent_type:
            agent_clients_total = db.query(Contract.client_id).filter(Contract.agent_type == agent_type).distinct().count()

        # 6. Клиенты агента за год
        agent_clients_by_year = 0
        if agent_type and year:
            year_str = str(year)
            agent_clients_by_year = db.query(Contract.client_id).filter(Contract.agent_type == agent_type, Contract.contract_date.like(f"{year_str}-%")).distinct().count()
...
@router.get("/crm")
async def get_crm_dashboard(project_type: str, agent_type: Optional[str] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        total_query = db.query(func.count(Contract.id), func.coalesce(func.sum(Contract.area), 0)).filter(contract_condition)
        ...
        if project_type == "Авторский надзор":
            ...
            active_orders = db.query(crm_table).join(Contract, crm_table.contract_id == Contract.id).filter(contract_condition).count()
            # Архивные = надзорные карточки с завершённым контрактом
            archive_orders = db.query(crm_table).join(Contract, crm_table.contract_id == Contract.id).filter(Contract.status.in_([STATUS_COMPLETED, STATUS_TERMINATED])).count()
        else:
            active_orders = db.query(crm_table).join(Contract, crm_table.contract_id == Contract.id).filter(contract_condition, ~Contract.status.in_(archive_statuses)).count()
            archive_orders = db.query(crm_table).join(Contract, crm_table.contract_id == Contract.id).filter(contract_condition, Contract.status.in_(archive_statuses)).count()

        # 5. Активные заказы агента (без архивных)
        agent_active_orders = 0
        if agent_type:
            if project_type == "Авторский надзор":
                agent_active_orders = db.query(crm_table).join(Contract, crm_table.contract_id == Contract.id).filter(contract_condition, Contract.agent_type == agent_type).count()
            else:
                agent_active_orders = (
                    db.query(crm_table)
                    .join(Contract, crm_table.contract_id == Contract.id)
                    .filter(contract_condition, Contract.agent_type == agent_type, ~Contract.status.in_(archive_statuses))
                    .count()
                )
        ...
        if agent_type:
            if project_type == "Авторский надзор":
                agent_archive_orders = (
                    db.query(crm_table)
                    .join(Contract, crm_table.contract_id == Contract.id)
                    .filter(Contract.status.in_([STATUS_COMPLETED, STATUS_TERMINATED]), Contract.agent_type == agent_type)
                    .count()
                )
            else:
                agent_archive_orders = (
...
async def get_employees_dashboard(current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        active_employees = db.query(Employee).filter(func.lower(Employee.status) == "активный").count()

        # 2. Резерв
        reserve_employees = db.query(Employee).filter(func.lower(Employee.status) == "в резерве").count()

        # 3. Руководящий состав (по должностям, как во вкладках UI)
        admin_positions = ADMIN_POSITIONS
        active_management = db.query(Employee).filter(func.lower(Employee.status) == "активный", Employee.position.in_(admin_positions)).count()

        # 4. Проектный отдел (по должностям)
        project_positions = ["Дизайнер", "Чертёжник"]
        active_projects_dept = db.query(Employee).filter(func.lower(Employee.status) == "активный", Employee.position.in_(project_positions)).count()

        # 5. Исполнительный отдел (по должностям)
        exec_positions = EXEC_POSITIONS
        active_execution_dept = db.query(Employee).filter(func.lower(Employee.status) == "активный", Employee.position.in_(exec_positions)).count()
        ...
        employees = db.query(Employee).filter(func.lower(Employee.status) == "активный").all()
...
@router.get("/salaries")
async def get_salaries_dashboard(year: Optional[int] = None, month: Optional[int] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    """Получить статистику для дашборда страницы Зарплаты"""
    try:
        # 1. Всего выплачено (все время)
        total_paid = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid").scalar()

        # 2. Выплачено за год
        paid_by_year = 0
        if year:
            paid_by_year = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%")).scalar()

        # 3. Выплачено за месяц
        paid_by_month = 0
        if year and month:
            paid_by_month = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid", Payment.report_month == f"{year}-{month:02d}").scalar()
        ...
        if year:
            individual_by_year = (
                ...
                .join(Contract)
                .filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%"), Contract.project_type == "Индивидуальный")
                .scalar()
            )
        ...
        if year:
            template_by_year = (
                db.query(func.coalesce(func.sum(Payment.final_amount), 0))
                .join(Contract)
                .filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%"), Contract.project_type == "Шаблонный")
                .scalar()
            )
        ...
        if year:
            supervision_by_year = (
                db.query(func.coalesce(func.sum(Payment.final_amount), 0))
                .join(Contract)
                .filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%"), Contract.status == STATUS_SUPERVISION)
                .scalar()
            )
...
async def get_salaries_by_type_dashboard(
    payment_type: str, year: Optional[int] = None, month: Optional[int] = None, agent_type: Optional[str] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if payment_type == "salary":
            ...
            total_paid = db.query(func.coalesce(func.sum(Salary.amount), 0)).scalar()

            # За год
            paid_by_year = 0
            if year:
                paid_by_year = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.report_month.like(f"{year}-%")).scalar()

            # За месяц
            paid_by_month = 0
            if year and month:
                paid_by_month = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.report_month == f"{year}-{month:02d}").scalar()
            ...
            if year:
                ...
                if query_filter:
                    ...
                    for f in query_filter:
                        q = q.filter(f)
                paid_by_year = q.filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%")).scalar()
            ...
            if year and month:
                ...
                if query_filter:
                    ...
                    for f in query_filter:
                        q = q.filter(f)
                paid_by_month = q.filter(Payment.payment_status == "paid", Payment.report_month == f"{year}-{month:02d}").scalar()
            ...
            if agent_type:
                ...
                if query_filter:
                    for f in query_filter:
                        q = q.filter(f)
                by_agent = q.filter(Payment.payment_status == "paid", Contract.agent_type == agent_type).scalar()
...
async def get_salaries_all_dashboard(year: Optional[int] = None, month: Optional[int] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        payments_total = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid").scalar()
        salaries_total = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid").scalar()
        ...
        if year:
            # Payments за год
            payments_year = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%")).scalar()
            # Salaries за год
            salaries_year = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%")).scalar()
            ...
            ind_payments = (
                db.query(func.coalesce(func.sum(Payment.final_amount), 0))
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%"), Contract.project_type == "Индивидуальный")
                .scalar()
            )
            ind_salaries = (
                db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Индивидуальный").scalar()
            )
            ...
            tmpl_salaries = (
                db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Шаблонный").scalar()
            )
            ...
            sup_payments = (
                db.query(func.coalesce(func.sum(Payment.final_amount), 0))
                .filter(Payment.payment_status == "paid", Payment.report_month.like(f"{year}-%"), Payment.supervision_card_id.isnot(None))
                .scalar()
            )
            sup_salaries = (
                db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Авторский надзор").scalar()
            )
        ...
        paid_by_month = 0
        if year and month:
            payments_month = db.query(func.coalesce(func.sum(Payment.final_amount), 0)).filter(Payment.payment_status == "paid", Payment.report_month == f"{year}-{month:02d}").scalar()
            salaries_month = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month == f"{year}-{month:02d}").scalar()
...
async def get_salaries_individual_dashboard(
    year: Optional[int] = None, month: Optional[int] = None, agent_type: Optional[str] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        total_result = (
            db.query(func.coalesce(func.sum(Payment.final_amount), 0), func.coalesce(func.avg(Payment.final_amount), 0))
            .join(CRMCard, Payment.crm_card_id == CRMCard.id)
            .join(Contract, CRMCard.contract_id == Contract.id)
            .filter(Payment.payment_status == "paid", Contract.project_type == "Индивидуальный")
            .first()
        )
        total_paid = float(total_result[0] or 0)
        avg_payment = total_result[1]
        # + оклады
        total_paid += float(db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.project_type == "Индивидуальный").scalar() or 0)
        ...
        if year:
            year_result = (
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Индивидуальный", Payment.report_month.like(f"{year}-%"))
                .first()
            )
            ...
            # + оклады за год
            sal_year = (
                db.query(func.coalesce(func.sum(Salary.amount), 0), func.count(Salary.id))
                .filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Индивидуальный")
                .first()
            )
        ...
        if year and month:
            paid_by_month = float(
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Индивидуальный", Payment.report_month == f"{year}-{month:02d}")
                .scalar()
                or 0
            )
            # + оклады за месяц
            paid_by_month += float(
                db.query(func.coalesce(func.sum(Salary.amount), 0))
                .filter(Salary.payment_status == "paid", Salary.report_month == f"{year}-{month:02d}", Salary.project_type == "Индивидуальный")
                .scalar()
                or 0
            )
        ...
        if agent_type:
            by_agent = (
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Индивидуальный", Contract.agent_type == agent_type)
                .scalar()
            )
...
async def get_salaries_template_dashboard(
    ...
    try:
        ...
        total_result = (
            ...
            .join(CRMCard, Payment.crm_card_id == CRMCard.id)
            .join(Contract, CRMCard.contract_id == Contract.id)
            .filter(Payment.payment_status == "paid", Contract.project_type == "Шаблонный")
            .first()
        )
        total_paid_payments = float(total_result[0] or 0)
        avg_payment = total_result[1]
        # + оклады
        total_paid_salaries = float(db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.project_type == "Шаблонный").scalar() or 0)
        ...
        if year:
            year_result = (
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Шаблонный", Payment.report_month.like(f"{year}-%"))
                .first()
            )
            paid_by_year = float(year_result[0] or 0)
            payments_count = int(year_result[1] or 0)
            ...
            sal_year = (
                db.query(func.coalesce(func.sum(Salary.amount), 0), func.count(Salary.id))
                .filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Шаблонный")
                .first()
            )
            paid_by_year += float(sal_year[0] or 0)
            payments_count += int(sal_year[1] or 0)
        ...
        if year and month:
            paid_by_month = float(
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Шаблонный", Payment.report_month == f"{year}-{month:02d}")
                .scalar()
                or 0
            )
            # + оклады за месяц
            paid_by_month += float(
                db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month == f"{year}-{month:02d}", Salary.project_type == "Шаблонный").scalar()
                or 0
            )
        ...
        if agent_type:
            by_agent = (
                ...
                .join(CRMCard, Payment.crm_card_id == CRMCard.id)
                .join(Contract, CRMCard.contract_id == Contract.id)
                .filter(Payment.payment_status == "paid", Contract.project_type == "Шаблонный", Contract.agent_type == agent_type)
                .scalar()
            )
...
async def get_salaries_salary_dashboard(
    ...
    try:
        ...
        total_result = db.query(func.coalesce(func.sum(Salary.amount), 0), func.coalesce(func.avg(Salary.amount), 0)).filter(Salary.payment_status == "paid").first()
        ...
        if year:
            year_result = (
                db.query(func.coalesce(func.sum(Salary.amount), 0), func.count(func.distinct(Salary.employee_id)))
                .filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"))
                .first()
            )
        ...
        paid_by_month = 0
        if year and month:
            paid_by_month = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.report_month == f"{year}-{month:02d}").scalar()

        # По типу проекта (только оплаченные)
        by_project_type = 0
        if project_type:
            by_project_type = db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.project_type == project_type).scalar()
...
async def get_salaries_supervision_dashboard(
    ...
    try:
        ...
        total_result = (
            db.query(func.coalesce(func.sum(Payment.final_amount), 0), func.coalesce(func.avg(Payment.final_amount), 0))
            .filter(Payment.payment_status == "paid", Payment.supervision_card_id.isnot(None))
            .first()
        )
        total_paid = float(total_result[0] or 0)
        avg_payment = total_result[1]
        # + оклады
        total_paid += float(db.query(func.coalesce(func.sum(Salary.amount), 0)).filter(Salary.payment_status == "paid", Salary.project_type == "Авторский надзор").scalar() or 0)
        ...
        if year:
            year_result = (
                db.query(func.coalesce(func.sum(Payment.final_amount), 0), func.count(Payment.id))
                .filter(Payment.payment_status == "paid", Payment.supervision_card_id.isnot(None), Payment.report_month.like(f"{year}-%"))
                .first()
            )
            paid_by_year = float(year_result[0] or 0)
            payments_count = int(year_result[1] or 0)
            ...
            sal_year = (
                db.query(func.coalesce(func.sum(Salary.amount), 0), func.count(Salary.id))
                .filter(Salary.payment_status == "paid", Salary.report_month.like(f"{year}-%"), Salary.project_type == "Авторский надзор")
                .first()
            )
            paid_by_year += float(sal_year[0] or 0)
            payments_count += int(sal_year[1] or 0)
        ...
        if year and month:
            paid_by_month = float(
                db.query(func.coalesce(func.sum(Payment.final_amount), 0))
                .filter(Payment.payment_status == "paid", Payment.supervision_card_id.isnot(None), Payment.report_month == f"{year}-{month:02d}")
                .scalar()
                or 0
            )
            # + оклады за месяц
            paid_by_month += float(
                db.query(func.coalesce(func.sum(Salary.amount), 0))
                .filter(Salary.payment_status == "paid", Salary.report_month == f"{year}-{month:02d}", Salary.project_type == "Авторский надзор")
                .scalar()
                or 0
            )
        ...
        if agent_type:
            by_agent = (
                ...
                .filter(Payment.payment_status == "paid", Payment.supervision_card_id.isnot(None), Contract.agent_type == agent_type)
                .scalar()
            )
...
async def get_agent_types(current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        agents = db.query(Contract.agent_type).filter(Contract.agent_type.isnot(None), Contract.agent_type != "").distinct().order_by(Contract.agent_type).all()
...
async def get_contract_years(current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        years_query = db.query(func.substr(Contract.contract_date, 1, 4).label("year")).filter(Contract.contract_date.isnot(None), Contract.contract_date != "").distinct().all()
...
async def get_reports_summary(
    ...
    try:
        ...
        for cid in client_ids_in_period:
            ...
            check = _apply_period_filter([type("C", (), {"contract_date": dt.strftime("%Y-%m-%d"), "client_id": cid})()], year, quarter, month)
            if check:
                new_clients += 1
        ...
        returning_clients = sum(1 for cid in client_ids_in_period if client_contract_count.get(cid, 0) > 1)
        ...
        if filtered_contract_ids:
            completed_cards = db.query(CRMCard).filter(CRMCard.contract_id.in_(filtered_contract_ids), CRMCard.column_name == "Выполненный проект").all()
            if completed_cards:
                on_time_count = 0
        ...
        if filtered_contract_ids:
            ...
            crm_card_ids = [row[0] for row in db.query(CRMCard.id).filter(CRMCard.contract_id.in_(filtered_contract_ids)).all()]
            if crm_card_ids:
                completed_stages = db.query(StageExecutor).filter(StageExecutor.crm_card_id.in_(crm_card_ids), StageExecutor.completed == True).all()
                if completed_stages:
                    on_time_stages = 0
...
async def get_clients_dynamics(year: Optional[int] = None, granularity: Optional[str] = "month", current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Исключаем Авторский надзор — для него отдельная секция
        all_contracts = db.query(Contract).filter(Contract.project_type != "Авторский надзор").all()
        ...
        period_data: dict = defaultdict(lambda: {"new_set": set(), "returning_set": set(), "individual_set": set(), "legal_set": set(), "all_set": set()})
...
async def get_crm_analytics(
    ...
    try:
        ...
        all_contracts = db.query(Contract).filter(Contract.project_type == project_type).all()
        ...
        if use_supervision:
            crm_cards = db.query(SupervisionCard).filter(SupervisionCard.contract_id.in_(filtered_ids)).all()
        else:
            crm_cards = db.query(CRMCard).filter(CRMCard.contract_id.in_(filtered_ids)).all()
        ...
        if not use_supervision and filtered_ids:
            timeline_entries = db.query(ProjectTimelineEntry).filter(ProjectTimelineEntry.contract_id.in_(filtered_ids)).all()
        ...
        active_count = sum(1 for c in crm_cards if c.column_name not in archived_statuses and c.column_name != "В ожидании")
...
async def get_supervision_analytics(
    year: Optional[int] = None, quarter: Optional[int] = None, month: Optional[int] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        by_city = [{"city": city, "count": cnt} for city, cnt in sorted(city_counter.items(), key=lambda x: -x[1])]
        ...
        stages_result = []
        if filtered_sc_ids:
            timeline_entries = db.query(SupervisionTimelineEntry).filter(SupervisionTimelineEntry.supervision_card_id.in_(filtered_sc_ids)).all()
        ...
        visits_supplier = sum(1 for v in visits_query if (v.visit_type or "") == "К поставщику")
        # Просрочки: плановая дата прошла, а фактической нет
        overdue_visits = sum(1 for v in visits_query if v.visit_date and v.visit_date < today_str and not v.actual_date)

        # По городам разбивка с типами
        city_visits: dict = {}  # city → {on_site: N, supplier: N}
        for v in visits_query:
            sc = sc_by_id.get(v.supervision_card_id)
            if sc is None:
                continue
            cont = contracts_map.get(sc.contract_id)
            city = (cont.city if cont else None) or "Не указан"
            if city not in city_visits:
                city_visits[city] = {"on_site": 0, "supplier": 0}
            vtype = v.visit_type or "На объект"
            if vtype == "На объект":
                city_visits[city]["on_site"] += 1
            else:
                city_visits[city]["supplier"] += 1

        by_city_visits = [
            {"city": city, "on_site": d["on_site"], "supplier": d["supplier"], "total": d["on_site"] + d["supplier"]}
            for city, d in sorted(city_visits.items(), key=lambda x: -(x[1]["on_site"] + x[1]["supplier"]))
        ]

        avg_visits = round(visits_total / total, 1) if total > 0 else 0
        ...
        return {
            ...
            "visits": {
                "total": visits_total,
                "on_site": visits_on_site,
                "supplier": visits_supplier,
                "overdue": overdue_visits,
                "avg_per_card": avg_visits,
                "by_city": by_city_visits,
            },
        }
...
async def get_distribution(
    dimension: str, year: Optional[int] = None, quarter: Optional[int] = None, month: Optional[int] = None, current_user: Employee = Depends(get_current_user), db: Session = Depends(get_db)
):
