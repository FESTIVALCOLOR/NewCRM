...
Пользователь → /orkester <задача>
 ...
 Worker (opus) → код + делегирование спец. агентам
     │
     ▼
 QA Monitor (sonnet) → запуск CRM + инструкции + мониторинг крашей [если UI]
     │
     ▼
 Test-Runner (haiku) ⟷ Debugger (sonnet) [макс 3 цикла]
     │
     ▼
 CI Push & Verify → git push + GitHub Actions [макс 3 CI-Fix цикла]
     │
     ▼
...
| 17 | **QA Monitor** | sonnet | Ручное QA | Запуск CRM, инструкции, мониторинг крашей |

**Итого:** opus ×4, sonnet ×9, haiku ×4
...
### Цикл F: CI Push & Verify (макс 3 итерации)
```
git push → GitHub Actions запускается → ожидание (макс 10 мин) →
  ├─ CI PASSED → продолжить конвейер
  └─ CI FAILED → Debugger анализирует логи CI → исправляет → повторный push
       ├─ OK → продолжить
       └─ FAIL → повторить (макс 3) → ЭСКАЛАЦИЯ

CI включает 5 jobs: syntax-check, lint, test-db, docker-build, test-e2e
Команды: gh run list, gh run view, gh run view --log-failed
```

---

## 7 режимов оркестрации

| Режим | Ключевые слова | Фазы |
|-------|---------------|------|
| **full** | добавить, реализовать, создать, новый | Planner→Worker→**QA Monitor**→Test→**CI Push**→Review→Compat→Security?→Doc→SeniorReview? |
| **fix** | исправь, баг, не работает, ошибка | Planner(lite)→Debugger→**QA Monitor?**→Test→**CI Push**→Review?→Compat? |
| **test** | тесты, проверь, запусти, покрытие | Test-Runner→Debugger?→**CI Push**→Doc(отчёт) |
| **refactor** | рефакторинг, улучши, упрости | Planner→SeniorReview→Refactor→**QA Monitor**→Test→**CI Push**→Review→Doc |
| **security** | безопасность, аудит, уязвимость | SecurityAuditor→Planner→Worker→Test→**CI Push**→Doc |
| **deploy** | деплой, deploy, обнови сервер | Compat→Test(critical)→**CI Push**→Deploy→Test(smoke)→Doc |
