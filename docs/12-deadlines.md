...
### Не реализовано (TODO) — Фаза 8

#### Фаза 8.0 (CRITICAL) — Привязка дедлайнов к системе
- [ ] Дедлайн дизайнера → read-only QLabel + кнопка "Изменить" с причиной
- [ ] Дедлайн чертёжника → аналогично
- [ ] Дедлайн проекта → авто-расчёт START + contract_term

#### Фаза 8.1 (CRITICAL) — Синхронизация UI после workflow
- [ ] Обновление timeline_widget после workflow_submit/accept/reject/client_send/client_ok
- [ ] Серверные исправления: workflow_reject → обновлять timeline, client_ok → заполнять дату

#### Фаза 8.2 (HIGH) — Отображение превышения нормо-дней
- [ ] Поле custom_norm_days в ProjectTimelineEntry
- [ ] Зачёркнутая стандартная + красная кастомная в таблице
- [ ] Установка при назначении исполнителя (ExecutorSelectionDialog)

#### Фаза 8.3 (HIGH) — Пропуск промежуточных дат
- [ ] Логика пропуска на сервере при "Согласовать с клиентом"
- [ ] Отображение пропущенных стадий в UI (серый фон)

#### Фаза 8.4 (MEDIUM) — Динамические кнопки подэтапов
- [ ] Маппинг кнопок → текущий подэтап
- [ ] Определение текущего подэтапа из timeline

#### Фаза 8.5 (LOW) — Улучшения
- [ ] Валидация sum(norm_days) в админке
- [ ] Webhook на изменение шаблона
- [ ] Гибкий маппинг stage_name → stage_group
