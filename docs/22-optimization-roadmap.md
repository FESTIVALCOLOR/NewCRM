...
> Обновлён после Phase 0-7.5 аудита (включая stabilization & signal safety)

## Общая оценка проекта: 9.5/10 (было 7/10)

| Метрика | Было | Стало |
|---------|------|-------|
| Основной код | 80.1K строк | ~130K строк |
| С тестами | 116.5K строк | ~160K строк |
| API endpoints | 183 | **214** (в 23 роутерах) |
| Таблицы PostgreSQL | 23 | **29** |
| Миграции SQLite | 44 | **50+** |
| E2E тесты | 30+ файлов | **71+ файлов, 1526+ тестов** |
| Документация | 21+ документ | **22 документа** |
| Безопасность | ~78% | **~96%** |
| DataAccess adoption | 3% | **100%** |
| Offline write coverage | 0 | **34 метода (100%)** |
| Segfaults | Множественные | **0** (Phase 7.5) |
| Offline queue бизнес-ошибки | Попадали | **Отфильтрованы** (Phase 7.5) |
...
**Контекст:** После Phase 6.3 все 34 write-метода используют local-first + offline queue через OfflineManager (18 entity types). Phase 7.5 добавила фильтрацию бизнес-ошибок (409/400 НЕ ставятся в очередь). Conflict resolution — следующий логический шаг.
...
- [x] **1.5.1** ~~Убрать fallback-значение SECRET_KEY в config.py~~ → **DONE Phase 5.1** (secrets.token_hex(32) по умолчанию)
- [ ] **1.5.2** Добавить ротацию JWT secret key (раз в квартал)
- [x] **1.5.3** ~~Добавить rate limit для /api/auth/login~~ → **DONE Phase 1** (5/min login, 300/min глобально)
...
### ~~3.1 Разделение server/main.py на роутеры~~ — DONE Phase 2+3

**ВЫПОЛНЕНО:** 22 роутера + 2 сервиса, main.py 424 строки (−96.2%)
...
- [x] **3.1.1-3.1.9** Все выполнены в Phase 2+3. 22 роутера создано, main.py = 424 строки.

**Правило:** Каждый роутер — один коммит. Тесты после каждого шага.

---

### ~~3.2 Декомпозиция crm_tab.py (17834 строк)~~ — DONE Phase 4
...
- [x] **3.2.1-3.2.6** Выполнено в Phase 4. crm_tab.py: 17 673 → 3 368 (−81%). Выделены: crm_card_edit_dialog.py (8201), crm_dialogs.py (4871), crm_archive.py (1389). Аналогично supervision: 7 721 → 2 246 (−71%), contracts: 5 030 → 693 (−86%).

---

### 3.3 Service layer на сервере — ЧАСТИЧНО

**Проблема:** Бизнес-логика внутри endpoint handlers

- [ ] **3.3.1** Создать `server/services/crm_service.py` — ОТКРЫТ
- [ ] **3.3.2** Создать `server/services/payment_service.py` — ОТКРЫТ
- [x] **3.3.3** ~~Создать `server/services/notification_service.py`~~ → **DONE Phase 3** (346 строк + timeline_service.py 413 строк)

---

### 3.4 Единая схема SQLite / PostgreSQL — ЧАСТИЧНО

**Проблема:** 2 источника истины для схемы БД

- [x] **3.4.1** ~~Внедрить Alembic для PostgreSQL миграций~~ → **DONE Phase 2** (server/alembic/)
...
### ~~3.8 SQL Injection аудит~~ — DONE Phase 5+5.1

- [x] **3.8.1** ~~Проверить все вызовы `execute_raw_query()`~~ → **DONE Phase 5** (ALLOWED_TABLES + _validate_table)
- [x] **3.8.2** ~~f-string SET clause~~ → **DONE Phase 5.1** (_validate_columns + _build_set_clause, 11 UPDATE методов)
...
| Фаза | Метрика | Цель | Текущее |
|------|---------|------|---------|
| 1 | Потеря данных | 0 инцидентов | 0 ✅ |
| 1 | Успешность бэкапов | 99.9% | Настроены (cron) |
| 2 | Время на типовую операцию | -30% | Открыт |
| 2 | Количество кликов (создание карточки) | -20% | Открыт |
| 3 | Максимальный размер файла | < 3000 строк | **5 203** (db_manager.py) |
| 3 | Покрытие тестами | > 60% | ~45% (оценка) |
| 4 | Время отклика API (p95) | < 200ms | Открыт |
| 4 | Доступность сервера | 99.5%+ | Открыт |
