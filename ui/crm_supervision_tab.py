...
from PyQt5.QtCore import QDate, QMimeData, QSize, Qt, QTimer, QUrl, pyqtSignal
...
class CRMSupervisionTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Архив — только для имеющих право просмотра архива
        if _has_perm(self.employee, self.api_client, "supervision.view_archive"):
    ...
    def on_card_moved(self, card_id, from_column, to_column):
        ...
        try:
            ...
            if to_column not in non_work_columns and not self.is_dan_role:
                ...
                executors_data = None
                if card_data:
                    executors_data = {"dan_id": card_data.get("dan_id"), "senior_manager_id": card_data.get("senior_manager_id")}
            ...
            if not self.is_dan_role and from_column not in ["Новый заказ", "В ожидании", "Выполненный проект"]:
                ...
                card_info = None
                if card_data:
                    card_info = {"dan_completed": card_data.get("dan_completed", 0), "dan_name": card_data.get("dan_name", "ДАН")}
                ...
                if card_info:
                    ...
                    if dan_completed == 0 and _has_perm(self.employee, self.api_client, "supervision.complete_stage"):
                        ...
                        self.data.add_supervision_history(
                            card_id, self.employee["id"], "accepted", f"Стадия '{from_column}' автоматически принята при перемещении руководством. Исполнитель: {dan_name}"
                        )
                        ...
                        if contract_id:
                            ...
                            if not existing_payments:
                                ...
                                executors_row = None
                                if card_api_data:
                                    executors_row = {"dan_id": card_api_data.get("dan_id"), "senior_manager_id": card_api_data.get("senior_manager_id")}
                                ...
                                if executors_row:
                                    ...
                                    if executors_row["dan_id"]:
                                        ...
                                        dan_amount = 0
                                        try:
                                            dan_amount = self.data.calculate_payment_amount(contract_id, executors_row["dan_id"], "ДАН", stage_name=from_column, supervision_card_id=card_id)
                                        except Exception as e:
                                    ...
                                    if executors_row["senior_manager_id"]:
                                        ...
                                        try:
                                            smp_amount = self.data.calculate_payment_amount(
                                                contract_id, executors_row["senior_manager_id"], "Старший менеджер проектов", stage_name=from_column, supervision_card_id=card_id
                                            )
                                        except Exception as e:
...
class SupervisionCard(QFrame):
    ...
    def init_ui(self):
        ...
        if not self.is_dan_role and self.card_data.get("dan_completed"):
            work_done_label = QLabel(f"Работа сдана: {self.card_data.get('dan_name', 'ДАН')}\nТребуется согласование и перемещение на следующую стадию")
    ...
    def pause_card(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if reason:
                self.data.pause_supervision_card(self.card_data["id"], reason)
    ...
    def resume_card(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            self.data.resume_supervision_card(self.card_data["id"], self.employee["id"])
            self.refresh_parent_tab()
    ...
    def submit_work(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            try:
                ...
                self.data.add_supervision_history(self.card_data["id"], self.employee["id"], "submitted", f"Стадия '{column_name}' сдана на проверку")
    ...
    def accept_work(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            try:
                self.data.add_supervision_history(self.card_data["id"], self.employee["id"], "accepted", f"Стадия '{column_name}' принята менеджером. Исполнитель: {dan_name}")
...
from ui.supervision_dialogs import (
    AddProjectNoteDialog,
    AssignExecutorsDialog,
    PauseDialog,
    SupervisionCompletionDialog,
    SupervisionFileUploadDialog,
    SupervisionReassignDANDialog,
    SupervisionStageDeadlineDialog,
    SupervisionStatisticsDialog,
)
