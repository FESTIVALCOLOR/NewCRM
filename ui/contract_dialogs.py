...
from PyQt5.QtGui import QCursor, QDesktopServices, QValidator
...
from config import AGENTS, PROJECT_SUBTYPES, PROJECT_TYPES, TEMPLATE_SUBTYPES, YANDEX_DISK_TOKEN
...
from utils.table_settings import ProportionalResizeTable, TableSettings
...
class ContractDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for unsigned_prefix, _, row_label in _act_rows:
            ...
            up_btn.clicked.connect(lambda checked, p=unsigned_prefix, lbl=row_label: self._upload_doc_file(p, lbl))
            ...
            del_btn.clicked.connect(lambda checked, p=unsigned_prefix, lbl=row_label: self._delete_doc_file(p, lbl))
        ...
        for _, signed_prefix, row_label in _act_rows:
            ...
            up_btn = IconLoader.create_icon_button("download", "", "Загрузить файл", icon_size=14)
            up_btn.setFixedSize(28, 28)
            up_btn.clicked.connect(lambda checked, p=signed_prefix, lbl=row_label: self._upload_doc_file(p, lbl))
            up_btn.setStyleSheet(_upload_btn_style)
            setattr(self, f"{signed_prefix}_upload_btn", up_btn)

            del_btn = IconLoader.create_icon_button("delete2", "", "Удалить файл", icon_size=14)
            del_btn.setFixedSize(28, 28)
            del_btn.clicked.connect(lambda checked, p=signed_prefix, lbl=row_label: self._delete_doc_file(p, lbl))
            del_btn.setStyleSheet(_delete_btn_style)
            del_btn.setVisible(False)
        ...
        for data_prefix, _, row_label in _tpl_act_rows:
            ...
            up_btn = IconLoader.create_icon_button("download", "", "Загрузить файл", icon_size=14)
            up_btn.setFixedSize(28, 28)
            up_btn.clicked.connect(lambda checked, p=data_prefix, lbl=row_label: self._upload_doc_file(p, lbl))
            up_btn.setStyleSheet(_upload_btn_style)
            setattr(self, f"{widget_prefix}_upload_btn", up_btn)

            del_btn = IconLoader.create_icon_button("delete2", "", "Удалить файл", icon_size=14)
            del_btn.setFixedSize(28, 28)
            del_btn.clicked.connect(lambda checked, p=data_prefix, lbl=row_label: self._delete_doc_file(p, lbl))
            del_btn.setStyleSheet(_delete_btn_style)
            del_btn.setVisible(False)
        ...
        self.survey_comment_label.setStyleSheet("color: #555; font-size: 11px; padding: 4px 8px; background: #F5F5F5; border-radius: 4px;")
        ...
        if not self.view_only:
            ...
            if self.contract_data and self._is_superuser():
                fix_btn = IconLoader.create_action_button(
                    "tool", "Диагностика и починка папки ЯД + загрузка файлов", bg_color="#5DADE2", hover_color="#3498DB", icon_color="#FFFFFF", icon_size=14, button_size=36
                )
        ...
        if self.view_only:
            ...
            managed_delete_buttons = [self.contract_file_delete_btn, self.template_contract_file_delete_btn, self.tech_task_file_delete_btn]
            ...
            for button in self.findChildren(QPushButton):
                ...
                if (
                    "загрузить pdf" in tooltip_lower
                    or "загрузить файл" in tooltip_lower
                    or "загрузить чек" in tooltip_lower
                    or "отметить как оплачено" in tooltip_lower
                    or "удалить файл" in tooltip_lower
                    or "удалить чек" in tooltip_lower
                    or button_text == "Добавить"
                    or button_text == "Таблица сроков"
                    or button_text == "Вручную"
                    or "управление" in button_tooltip.lower()
                ):
    ...
    def _show_template_term_table_dialog(self):
        ...
        desc = QLabel(
            "Срок договора (рабочих дней) для шаблонных проектов.\n"
        ...
        tbl.setHorizontalHeaderLabels(["Площадь (м\u00b2)", "Стандарт", "Стандарт+Визуал.", "Ванная", "Ванная+Визуал."])
    ...
    def _load_surveys(self):
        ...
        if status == "completed":
            ...
            if not has_any_score:
                ...
                self.survey_status_label.setText(f"Опрос заполнен ({completed_at}) — данные не получены (тестовый опрос)")
    ...
    def _create_survey(self):
        ...
        try:
            ...
            if result and result.get("id"):
                CustomMessageBox(self, "Успешно", "Опрос создан. Ссылка скопирована в буфер обмена.", "info").exec_()
    ...
    def _copy_survey_link(self):
        ...
        if self._current_survey_link:
            ...
            CustomMessageBox(self, "Скопировано", "Ссылка на опрос скопирована в буфер обмена.", "info").exec_()
                ...
                try:
                    ...
                    if local_contract:
                        _new_fields = [
                            ...
                            "advance_receipt_yandex_path",
                            "advance_receipt_file_name",
                            "additional_receipt_link",
                            "additional_receipt_yandex_path",
                            "additional_receipt_file_name",
                            "third_receipt_link",
                            "third_receipt_yandex_path",
                            "third_receipt_file_name",
                            "act_planning_signed_link",
                            "act_planning_signed_yandex_path",
                            "act_planning_signed_file_name",
                            "act_concept_signed_link",
                            "act_concept_signed_yandex_path",
                            "act_concept_signed_file_name",
                            "info_letter_signed_link",
                            "info_letter_signed_yandex_path",
                            "info_letter_signed_file_name",
                            "act_final_signed_link",
                            "act_final_signed_yandex_path",
                            "act_final_signed_file_name",
                        ]
                        ...
                        file_mappings.append(
                            {
                                ...
                                "db_fields": {"contract_file_link": "public_link", "contract_file_yandex_path": "yandex_path", "contract_file_name": "file_name"},
                            }
                        )
                else:
                        ...
                        file_mappings.append(
                            {
                                ...
                                "keywords": ["договор", "contract"],
                                "extensions": [".pdf", ".docx", ".doc"],
                                "db_fields": {"template_contract_file_link": "public_link", "template_contract_file_yandex_path": "yandex_path", "template_contract_file_name": "file_name"},
                            }
                        )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "extensions": [".pdf", ".docx", ".doc"],
                            "db_fields": {"tech_task_link": "public_link", "tech_task_yandex_path": "yandex_path", "tech_task_file_name": "file_name"},
                        }
                    )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "db_fields": {"measurement_image_link": "public_link", "measurement_yandex_path": "yandex_path", "measurement_file_name": "file_name"},
                        }
                    )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "extensions": [".pdf", ".docx", ".doc"],
                            "db_fields": {"act_planning_link": "public_link", "act_planning_yandex_path": "yandex_path", "act_planning_file_name": "file_name"},
                        }
                    )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "extensions": [".pdf", ".docx", ".doc"],
                            "db_fields": {"act_concept_link": "public_link", "act_concept_yandex_path": "yandex_path", "act_concept_file_name": "file_name"},
                        }
                    )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "extensions": [".pdf", ".docx", ".doc"],
                            "db_fields": {"info_letter_link": "public_link", "info_letter_yandex_path": "yandex_path", "info_letter_file_name": "file_name"},
                        }
                    )
                    ...
                    file_mappings.append(
                        {
                            ...
                            "extensions": [".pdf", ".docx", ".doc"],
                            "db_fields": {"act_final_link": "public_link", "act_final_yandex_path": "yandex_path", "act_final_file_name": "file_name"},
                        }
                    )
                        ...
                        file_mappings.append(
                            {
                                ...
                                "db_fields": {f"{signed_prefix}_link": "public_link", f"{signed_prefix}_yandex_path": "yandex_path", f"{signed_prefix}_file_name": "file_name"},
                            }
                        )
                        ...
                        file_mappings.append(
                            {
                                ...
                                "db_fields": {f"{receipt_prefix}_link": "public_link", f"{receipt_prefix}_yandex_path": "yandex_path", f"{receipt_prefix}_file_name": "file_name"},
                            }
                        )
    ...
    def upload_contract_file(self, project_type):
        ...
        if not contract_folder:
            CustomMessageBox(self, "Ошибка", "Сначала сохраните договор, чтобы создать папку на Яндекс.Диске", "warning").exec_()
            return
        ...
        def upload_thread():
            try:
                ...
                result = yd.upload_file_to_contract_folder(file_path, contract_folder, "Документы", file_name, progress_callback=update_progress)
                ...
                if result:
                    ...
                    self.contract_file_upload_completed.emit(result["public_link"], result["yandex_path"], result["file_name"], project_type)
                else:
    ...
    def _on_contract_file_uploaded(self, public_link, yandex_path, file_name, project_type):
        ...
        if public_link:
            ...
            if self.contract_data and self.contract_data.get("id"):
                ...
                try:
                    if project_type == "individual":
                        update_data = {"contract_file_link": public_link, "contract_file_yandex_path": yandex_path, "contract_file_name": file_name}
                    else:
                        update_data = {"template_contract_file_link": public_link, "template_contract_file_yandex_path": yandex_path, "template_contract_file_name": file_name}
                    self.data.update_contract(contract_id, update_data)
        ...
        def upload_thread():
            try:
                ...
                if result:
                    ...
                    QMetaObject.invokeMethod(progress, "setValue", Qt.QueuedConnection, Q_ARG(int, 3))
                    QMetaObject.invokeMethod(progress, "close", Qt.QueuedConnection)
                    self.tech_task_upload_completed.emit(result["public_link"], result["yandex_path"], result["file_name"])
                else:
                    from PyQt5.QtCore import QMetaObject, Qt
    ...
    def _on_tech_task_file_uploaded(self, public_link, yandex_path, file_name):
        ...
        if public_link:
            ...
            if self.contract_data and self.contract_data.get("id"):
                ...
                try:
                    update_data = {"tech_task_link": public_link, "tech_task_yandex_path": yandex_path, "tech_task_file_name": file_name}
                    self.data.update_contract(contract_id, update_data)
                    self.contract_data = self.data.get_contract(contract_id)
                except Exception as e:
                    print(f"[WARNING] Ошибка обновления contract через DataAccess: {e}")
    ...
    def _upload_doc_file(self, prefix, display_name):
        ...
        progress = create_progress_dialog("Загрузка файлов" if is_multi else "Загрузка файла", "Подготовка к загрузке...", "Отмена", total_steps, self)
        ...
        def upload_thread():
            try:
                ...
                for idx, fp in enumerate(file_paths):
                    ...
                    result = yd.upload_file_to_contract_folder(fp, contract_folder, doc_subfolder, fname, progress_callback=update_progress)
                    if result:
                        last_result = result
                ...
                if last_result:
                    ...
                    if is_multi:
                        ...
                        if folder_link:
                            ...
                            self.doc_file_upload_completed.emit(prefix, folder_link, folder_path, display)
                        else:
                            self.doc_file_upload_completed.emit(prefix, last_result["public_link"], last_result["yandex_path"], last_result["file_name"])
                    else:
                        self.doc_file_upload_completed.emit(prefix, last_result["public_link"], last_result["yandex_path"], last_result["file_name"])
                else:
                    from PyQt5.QtCore import QMetaObject, Qt
    ...
    def _on_doc_file_uploaded(self, prefix, public_link, yandex_path, file_name):
        ...
        if public_link:
            ...
            if self.contract_data and self.contract_data.get("id"):
                contract_id = self.contract_data["id"]
                update_data = {f"{prefix}_link": public_link, f"{prefix}_yandex_path": yandex_path, f"{prefix}_file_name": file_name}
                try:
                    self.data.update_contract(contract_id, update_data)
    ...
    def _try_auto_archive_after_payment(self, prefix):
        ...
        try:
            ...
            CustomMessageBox(self, "Проект архивирован", "<b>Финальный платёж подтверждён!</b><br><br>Карточка проекта автоматически перемещена в архив.", "success").exec_()
    ...
    def _upload_receipt_file(self, prefix):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, f"Выберите файл: {display_name}", "", "Изображения и PDF (*.pdf *.jpg *.jpeg *.png);;Все файлы (*.*)")
        if not file_path:
            return
        ...
        def upload_thread():
            try:
                ...
                if result:
                    ...
                    QMetaObject.invokeMethod(progress, "setValue", Qt.QueuedConnection, Q_ARG(int, 3))
                    QMetaObject.invokeMethod(progress, "close", Qt.QueuedConnection)
                    self.doc_file_upload_completed.emit(receipt_prefix, result["public_link"], result["yandex_path"], result["file_name"])
                else:
                    from PyQt5.QtCore import QMetaObject, Qt
    ...
    def _delete_receipt_file(self, payment_prefix):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", "Удалить загруженный чек?", "Удалить", "Отмена").exec_()
    ...
    def _delete_doc_file(self, prefix, display_name):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", f'Вы уверены, что хотите удалить файл "{display_name}"?').exec_()
        ...
        update_data = {f"{prefix}_link": "", f"{prefix}_yandex_path": "", f"{prefix}_file_name": ""}
        try:
            self.data.update_contract(contract_id, update_data)
    ...
    def delete_contract_file(self):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить файл договора?").exec_()
    ...
    def delete_tech_task_file_contracts(self):
        ...
        # Обновляем БД через DataAccess после успешного удаления с ЯД
        update_data = {"tech_task_link": "", "tech_task_yandex_path": "", "tech_task_file_name": ""}
        try:
            self.data.update_contract(contract_id, update_data)
    ...
    def _fix_contract_folder(self):
        ...
        try:
            ...
            try:
                fresh = self.data.get_contract(contract_id)
                if fresh:
    ...
    def save_contract(self):
        ...
        if self.client_combo.currentData() is None:
            CustomMessageBox(self, "Ошибка", "Выберите клиента из списка.\n\nНачните вводить имя клиента и выберите из выпадающего списка.", "warning").exec_()
            return
        ...
        if existing:
            # ========== ЗАМЕНИЛИ QMessageBox ==========
            CustomMessageBox(self, "Ошибка", f'Договор с номером "{contract_number}" уже существует!\n\nПожалуйста, укажите другой номер.', "error").exec_()
            return
        ...
        try:
            ...
            if self.contract_data:
                ...
                folder_changed = old_city != new_city or old_address != new_address or old_area != new_area or old_agent_type != new_agent_type or old_project_type != new_project_type
                ...
                if folder_changed and self.yandex_disk:
                    try:
                        new_folder_path = self.yandex_disk.build_contract_folder_path(agent_type=new_agent_type, project_type=new_project_type, city=new_city, address=new_address, area=new_area)
                ...
                if new_contract_id and contract_data.get("project_type") != "Авторский надзор":
                    try:
                        self.data._ensure_crm_card_exists({"id": new_contract_id}, contract_data)
                    except Exception as e:
                ...
                if new_contract_id and self.yandex_disk:
                    try:
                        ...
                        folder_path = self.yandex_disk.create_contract_folder_structure(agent_type=agent_type, project_type=project_type, city=city, address=address, area=area)
            ...
            if getattr(self, "_create_and_stay_mode", False):
                ...
                if new_contract_id:
                    ...
                    CustomMessageBox(self, "Создано", f"Договор №{contract_number} создан.\nТеперь можно добавить аванс и другие данные.", "success").exec_()
            ...
            if "UNIQUE constraint failed" in error_msg or "уже существует" in error_msg:
                CustomMessageBox(self, "Ошибка сохранения", f"Договор с таким номером уже существует!\n\nИзмените номер договора и попробуйте снова.", "error").exec_()
            else:
                CustomMessageBox(self, "Ошибка", f"Не удалось сохранить договор:\n{error_msg}", "error").exec_()
        finally:
...
class ContractSearchDialog(QDialog):
    ...
    def get_search_params(self):
        ...
        params = {"contract_number": self.contract_number_input.text().strip(), "address": self.address_input.text().strip(), "client_name": self.client_name_input.text().strip()}
