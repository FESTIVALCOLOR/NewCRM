...
class ContractDialog(QDialog):
    ...
    def init_ui(self):
        ...
        _paid_btn_style = '''
            QPushButton { background-color: #ffffff; border: 1px solid #d9d9d9; border-radius: 6px; padding: 0px 4px; }
    ...
    def fill_data(self):
        ...
        if self.contract_data and self.contract_data.get('id'):
            ...
            if self.db:
                try:
                    local_contract = self.db.get_contract_by_id(self.contract_data['id'])
                    if local_contract:
                        _new_fields = [
        ...
        def repair():
            try:
                ...
                for prefix, yandex_path in files_to_repair:
                    try:
                        result = self.api_client.get_yandex_public_link(yandex_path)
                        public_link = None
                ...
                for mapping in file_mappings:
                    ...
                    for item in items:
                        ...
                        if keyword_found:
                            ...
                            if self.api_client:
                                try:
                                    result = self.api_client.get_yandex_public_link(file_path)
                                    if isinstance(result, dict):
                                        public_link = result.get('public_url') or result.get('public_link')
                                    elif isinstance(result, str):
                                        public_link = result
                                except Exception as e:
                                    print(f"[SYNC] Ошибка получения ссылки через API: {e}")
                            if not public_link:
                                try:
    ...
    def refresh_file_labels(self):
        ...
        if contract_data:
            ...
            if self.db:
                try:
                    local = self.db.get_contract_by_id(contract_id)
                    if local:
                        _nf = [
                            'advance_payment_paid_date', 'additional_payment_paid_date', 'third_payment_paid_date',
                            'advance_receipt_link', 'advance_receipt_yandex_path', 'advance_receipt_file_name',
    ...
    def _upload_doc_file(self, prefix, display_name):
        ...
        progress.setFixedSize(420, 144)
        progress.setStyleSheet("""
            QProgressDialog { background-color: white; border: 1px solid #d9d9d9; border-radius: 10px; }
            QLabel { color: #2C3E50; font-size: 12px; padding: 10px; min-width: 380px; max-width: 380px; }
            QProgressBar { border: 1px solid #d9d9d9; border-radius: 6px; text-align: center; background-color: #F0F0F0; height: 20px; margin: 10px; min-width: 380px; max-width: 380px; }
            QProgressBar::chunk { background-color: #90EE90; border-radius: 2px; }
            QPushButton { background-color: #E74C3C; color: white; border: none; padding: 8px 16px; border-radius: 6px; min-width: 80px; }
            QPushButton:hover { background-color: #C0392B; }
        """)
        progress.show()
                ...
                try:
                    if self.api_client:
                        try:
                            ...
                            print(f"[WARN] API update {prefix}: {e}")
        ...
        try:
            ...
            if self.api_client and self.api_client.is_online:
                try:
                    ...
                    print(f"[WARN] API update paid_date: {e}")
            # Всегда сохраняем в локальную БД
            self.db.update_contract(contract_id, update_data)
    ...
    def _upload_receipt_file(self, prefix):
        ...
        progress = QProgressDialog("Подготовка к загрузке...", "Отмена", 0, 3, self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setWindowTitle("Загрузка чека")
        progress.setMinimumDuration(0)
        progress.setAutoClose(True)
        progress.setAutoReset(False)
        progress.setWindowFlags(Qt.FramelessWindowHint | Qt.Dialog)
        ...
        def upload_thread():
            try:
                ...
                if result:
                    ...
                    from PyQt5.QtCore import QTimer
                    QTimer.singleShot(0, progress.close)
                    # Используем тот же сигнал doc_file_upload_completed но с receipt_prefix
                    self.doc_file_upload_completed.emit(
    ...
    def _delete_receipt_file(self, payment_prefix):
        ...
        try:
            ...
            if yandex_path:
                try:
                    if self.api_client and self.api_client.is_online:
                        self.api_client.delete_yandex_file(yandex_path)
                    else:
                        yd = YandexDiskManager(YANDEX_DISK_TOKEN)
            ...
            # Очищаем поля в БД (двойная запись — API + локальная)
            update_data = {
            ...
            if self.api_client:
                try:
                    ...
                    print(f"[WARN] API delete receipt: {e}")
            # Всегда сохраняем в локальную БД
            self.db.update_contract(contract_id, update_data)
    ...
    def delete_contract_file(self):
        ...
        if contract_yandex_path:
            try:
                if self.api_client and self.api_client.is_online:
                    self.api_client.delete_yandex_file(contract_yandex_path)
                else:
                    yd = YandexDiskManager(YANDEX_DISK_TOKEN)
        ...
        if template_yandex_path:
            try:
                if self.api_client and self.api_client.is_online:
                    self.api_client.delete_yandex_file(template_yandex_path)
                else:
                    yd = YandexDiskManager(YANDEX_DISK_TOKEN)
    ...
    def save_contract(self):
        ...
        try:
            if self.contract_data:
                ...
                if self.api_client and self.api_client.is_online:
                    # API режим + локальная БД для надёжности
