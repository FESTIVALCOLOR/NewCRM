...
from utils.permissions import _emp_has_pos, _emp_only_pos, _has_perm, _load_user_permissions
...
class CardEditDialog(QDialog):
    ...
    def _add_action_history(self, action_type: str, description: str, entity_type: str = "crm_card", entity_id: int = None):
        ...
        try:
            self.data.add_action_history(user_id=user_id, action_type=action_type, entity_type=entity_type, entity_id=entity_id, description=description)
    ...
    def init_ui(self):
        ...
        if not is_executor:
            ...
            self._invite_sm_btn.clicked.connect(lambda: self._invite_employee_show_menu(self.senior_manager, "Старший менеджер", self._invite_sm_btn))
            ...
            if project_type == "Индивидуальный":
                ...
                self._invite_sdp_btn.clicked.connect(lambda: self._invite_employee_show_menu(self.sdp, "СДП", self._invite_sdp_btn))
            ...
            self._invite_gap_btn.clicked.connect(lambda: self._invite_employee_show_menu(self.gap, "ГАП", self._invite_gap_btn))
            ...
            self._invite_mgr_btn.clicked.connect(lambda: self._invite_employee_show_menu(self.manager, "Менеджер", self._invite_mgr_btn))
            ...
            if self.card_data.get("designer_name"):
                ...
                reassign_designer_btn.clicked.connect(lambda: self.reassign_executor_from_dialog("designer"))
                ...
                if has_deadlines_access:
                    ...
                    edit_designer_deadline_btn.setStyleSheet("""
                        ...
                        QPushButton:pressed { background-color: #C0C0C0; }
                    """)
                    edit_designer_deadline_btn.clicked.connect(lambda: self.change_executor_deadline("designer"))
            ...
            if self.card_data.get("draftsman_name"):
                ...
                reassign_draftsman_btn.clicked.connect(lambda: self.reassign_executor_from_dialog("draftsman"))
                ...
                if has_deadlines_access:
                    ...
                    edit_draftsman_deadline_btn.setStyleSheet("""
                        ...
                        QPushButton:pressed { background-color: #C0C0C0; }
                    """)
                    edit_draftsman_deadline_btn.clicked.connect(lambda: self.change_executor_deadline("draftsman"))
            ...
            self.senior_manager.currentIndexChanged.connect(lambda: self.on_employee_changed(self.senior_manager, "Старший менеджер проектов"))
            # ИСПРАВЛЕНИЕ 06.02.2026: СДП только для индивидуальных проектов (#20)
            if self.sdp:
                self.sdp.currentIndexChanged.connect(lambda: self.on_employee_changed(self.sdp, "СДП"))
            self.gap.currentIndexChanged.connect(lambda: self.on_employee_changed(self.gap, "ГАП"))
            self.manager.currentIndexChanged.connect(lambda: self.on_employee_changed(self.manager, "Менеджер"))
            self.surveyor.currentIndexChanged.connect(lambda: self.on_employee_changed(self.surveyor, "Замерщик"))
        ...
        # Чаты открываются из кнопок внизу карточки (не вкладки)
        ...
        if not self.view_only:
            ...
            if not is_executor:
                repair_btn = IconLoader.create_action_button(
                    "tool", "Диагностика и восстановление карточки", bg_color="#5DADE2", hover_color="#3498DB", icon_color="#FFFFFF", icon_size=14, button_size=36
                )
            ...
            if self.card_data.get("contract_id"):
                sync_btn = IconLoader.create_action_button(
                    "refresh-white", "Синхронизировать файлы с Яндекс.Диском", bg_color="#27AE60", hover_color="#219A52", icon_color="#FFFFFF", icon_size=14, button_size=36
                )
            ...
            # --- Кнопки внутренних чатов карточки ---
            self._has_chat_buttons = False
            self._need_chat_init = True  # Загружаем данные мессенджера для кнопок приглашения

            if _has_perm(self.employee, self.api_client, "chat.employee.view"):
                self._card_employee_chat_btn = IconLoader.create_icon_button("message-circle", "Чат сотрудников", "Открыть чат сотрудников по этой карточке", icon_size=14)
                self._card_employee_chat_btn.setStyleSheet("""
                    QPushButton {
                        background-color: #E8F5E9; color: #1B5E20;
                        padding: 0px 12px; border-radius: 4px; border: 1px solid #81C784;
                        font-weight: 500; max-height: 36px; min-height: 36px;
                    }
                    QPushButton:hover { background-color: #C8E6C9; border-color: #4CAF50; }
                    QPushButton:pressed { background-color: #A5D6A7; }
                """)
                self._card_employee_chat_btn.setFixedHeight(36)
                self._card_employee_chat_btn.clicked.connect(self._open_card_employee_chat)
                buttons_layout.addWidget(self._card_employee_chat_btn)

            if _has_perm(self.employee, self.api_client, "chat.client.view"):
                self._card_client_chat_btn = IconLoader.create_icon_button("message-circle", "Чат с клиентом", "Открыть чат с клиентом по этой карточке", icon_size=14)
                self._card_client_chat_btn.setStyleSheet("""
                    QPushButton {
                        background-color: #E3F2FD; color: #0D47A1;
                        padding: 0px 12px; border-radius: 4px; border: 1px solid #90CAF9;
                        font-weight: 500; max-height: 36px; min-height: 36px;
                    }
                    QPushButton:hover { background-color: #BBDEFB; border-color: #42A5F5; }
                    QPushButton:pressed { background-color: #90CAF9; }
                """)
                self._card_client_chat_btn.setFixedHeight(36)
                self._card_client_chat_btn.clicked.connect(self._open_card_client_chat)
                buttons_layout.addWidget(self._card_client_chat_btn)

            if False:  # Telegram-блок удалён; оставлен placeholder для совместимости атрибутов
                self._has_chat_buttons = True
                self.create_chat_btn = IconLoader.create_icon_button("message-circle", "Создать чат", "Создать чат в мессенджере", icon_size=14)
                ...
                self.open_chat_btn = IconLoader.create_icon_button("external-link", "Открыть чат", "Открыть чат в мессенджере", icon_size=14)
                ...
                self.delete_chat_btn = IconLoader.create_icon_button("trash", "Удалить чат", "Удалить чат из мессенджера", icon_size=14)
                ...
                self.invite_client_btn = IconLoader.create_icon_button("telegram", "", "Пригласить клиента — отправить email со ссылкой на чат", icon_size=16)
                ...
                self.start_script_btn = IconLoader.create_icon_button("play", "", "Начальный скрипт — отправить в чат", icon_size=16)
                ...
                self.end_script_btn = IconLoader.create_icon_button("check-circle", "", "Завершающий скрипт — отправить в чат", icon_size=16)
                ...
                # Telegram-блок удалён (оставлен placeholder для совместимости)
    ...
    def mark_survey_complete(self):
        ...
        if not surveyor_id:
            CustomMessageBox(self, "Ошибка", "Сначала выберите замерщика!", "warning").exec_()
            return
        ...
        """)
        save_btn.clicked.connect(lambda: self.save_survey_date(date_edit.date(), surveyor_id, dialog))
    ...
    def save_survey_date(self, survey_date, surveyor_id, dialog):
        ...
        try:
            ...
            try:
                ...
                existing_payment = next((p for p in payments if p.get("employee_id") == surveyor_id and p.get("role") == "Замерщик"), None)
                if existing_payment:
            ...
            self.data.update_contract(contract_id, {"measurement_date": survey_date.toString("yyyy-MM-dd")})
    ...
    def change_executor_deadline(self, executor_type):
        ...
        def save_executor_deadline():
            try:
                ...
                success = self.data.update_stage_executor_deadline(self.card_data["id"], search_key, new_deadline_str)
    ...
    def upload_project_tech_task_file_direct(self):
        ...
        def upload_thread():
            try:
                ...
                result = yd.upload_file_to_contract_folder(file_path, contract_folder, "Анкета", file_name, progress_callback=update_progress)
                ...
                if result:
                    ...
                    self.tech_task_upload_completed.emit(result["public_link"], result["yandex_path"], result["file_name"], contract_id)
                else:
                    QMetaObject.invokeMethod(progress, "close", Qt.QueuedConnection)
    ...
    def _on_project_tech_task_uploaded(self, public_link, yandex_path, file_name, contract_id):
        ...
        if public_link:
            ...
            result = self.data.update_contract(contract_id, {"tech_task_link": public_link, "tech_task_yandex_path": yandex_path, "tech_task_file_name": file_name})
            if result:
    ...
    def upload_references_files(self):
        ...
        def upload_thread():
            try:
                ...
                uploaded_files = yd.upload_stage_files(file_paths, contract_folder, "references", progress_callback=update_progress, skip_per_file_publish=True)
    ...
    def upload_photo_documentation_files(self):
        ...
        file_paths, _ = QFileDialog.getOpenFileNames(self, "Выберите файлы фотофиксации", "", "Images, PDF and Videos (*.png *.jpg *.jpeg *.pdf *.mp4 *.mov *.avi);;All Files (*.*)")
        ...
        def upload_thread():
            try:
                ...
                # Загружаем файлы (без публикации каждого — нужна только ссылка на папку)
                uploaded_files = yd.upload_stage_files(file_paths, contract_folder, "photo_documentation", progress_callback=update_progress, skip_per_file_publish=True)
    ...
    def delete_tech_task_file(self):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить файл ТЗ?").exec_()
        ...
        try:
            update_data = {"tech_task_link": None, "tech_task_yandex_path": None, "tech_task_file_name": None}
            self.data.update_contract(contract_id, update_data)
    ...
    def delete_references_folder(self):
        ...
        # Подтверждение удаления
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить ВСЕ референсы?\nВся папка будет удалена с Яндекс.Диска.").exec_()
    ...
    def delete_photo_documentation_folder(self):
        ...
        # Подтверждение удаления
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить ВСЮ фотофиксацию?\nВся папка будет удалена с Яндекс.Диска.").exec_()
    ...
    def add_project_templates(self):
        ...
        from PyQt5.QtWidgets import QDialog, QFrame, QHBoxLayout, QLabel, QLineEdit, QPushButton, QScrollArea, QVBoxLayout
    ...
    def delete_project_template(self, template_id):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить эту ссылку на шаблон?").exec_()
    ...
    def delete_measurement_file(self):
        ...
        # Подтверждение удаления
        reply = CustomQuestionBox(self, "Подтверждение", "Вы уверены, что хотите удалить изображение замера?").exec_()
        ...
        try:
            update_data = {"measurement_image_link": None, "measurement_yandex_path": None, "measurement_file_name": None, "measurement_date": None}
            self.data.update_contract(contract_id, update_data)
    ...
    def edit_measurement_date(self):
        ...
        def save_measurement_date():
            try:
                ...
                # Обновляем в БД - и crm_cards, и contracts
                updates = {"survey_date": date_str, "surveyor_id": surveyor_id}
                self.data.update_crm_card(self.card_data["id"], updates)
                self.card_data["survey_date"] = date_str
                self.card_data["surveyor_id"] = surveyor_id
    ...
    def create_project_data_widget(self):
        ...
        self.stage1_list = VariationGalleryWidget(title="PDF файлы планировочного решения", stage="stage1", file_types=["pdf"], can_delete=can_delete_stage1, can_upload=can_upload_stage1)
        ...
        if project_type == "Шаблонный":
            ...
            self.stage3_list = VariationGalleryWidget(  # используем stage3_list для совместимости с БД
                title="PDF и Excel файлы чертежного проекта", stage="stage3", file_types=["pdf", "excel"], can_delete=can_delete_stage2, can_upload=can_upload_stage2
            )
            ...
            self.stage2_3d_gallery = VariationGalleryWidget(title="3D визуализация", stage="stage2_3d", file_types=["image", "pdf"], can_delete=can_delete_stage3, can_upload=can_upload_stage3)
            ...
            self.stage2_concept_gallery = VariationGalleryWidget(
                title="Концепция-коллажи", stage="stage2_concept", file_types=["image", "pdf"], can_delete=can_delete_stage2, can_upload=can_upload_stage2
            )
            ...
            self.stage2_3d_gallery = VariationGalleryWidget(title="3D визуализация", stage="stage2_3d", file_types=["image", "pdf"], can_delete=can_delete_stage2, can_upload=can_upload_stage2)
            self.stage2_3d_gallery.upload_requested.connect(self.upload_stage_files_with_variation)
            self.stage2_3d_gallery.delete_requested.connect(self.delete_stage_file_with_variation)
            self.stage2_3d_gallery.add_variation_requested.connect(self.add_variation_folder)
            self.stage2_3d_gallery.delete_variation_requested.connect(self.delete_variation_folder)
            ...
            self.stage3_list = VariationGalleryWidget(
                title="PDF и Excel файлы чертежного проекта", stage="stage3", file_types=["pdf", "excel"], can_delete=can_delete_stage3, can_upload=can_upload_stage3
            )
            self.stage3_list.upload_requested.connect(self.upload_stage_files_with_variation)
            self.stage3_list.delete_requested.connect(self.delete_stage_file_with_variation)
            self.stage3_list.add_variation_requested.connect(self.add_variation_folder)
    ...
    def create_project_info_widget(self):
        ...
        try:
            ...
            completed_stages = [se for se in stage_executors if se.get("completed") or se.get("completed") == 1]
            ...
            if completed_stages:
                ...
                for stage in completed_stages:
                    ...
                    stage_label = QLabel(f"{stage.get('stage_name', '')} | Исполнитель: {stage.get('executor_name', '')} | Дата: {date_str}")
    ...
    def _on_history_filter_changed(self, filter_text):
        ...
        if filter_text == "Все действия":
            ...
            filtered_items = [a for a in self._all_history_items if a.get("action_type", "") not in all_known]
        elif allowed_types:
            filtered_items = [a for a in self._all_history_items if a.get("action_type", "") in allowed_types]
        else:
            filtered_items = self._all_history_items
                            ...
                            try:
                                ...
                                if not existing:
                                    ...
                                    result = self.data.create_supervision_card(
                                        {"contract_id": contract_id, "column_name": "Новый заказ", "start_date": QDate.currentDate().toString("yyyy-MM-dd"), "deadline": crm_deadline or ""}
                                    )
                    ...
                    if old_id is not None and new_id is not None and old_id != new_id:
                        try:
                            ...
                            for old_payment in old_payments:
                                ...
                                self.data.update_payment(old_payment["id"], {"reassigned": True})
    ...
    def on_employee_changed(self, combo_box, role_name):
        ...
        role_to_field = {"Старший менеджер проектов": "senior_manager_id", "СДП": "sdp_id", "ГАП": "gap_id", "Менеджер": "manager_id", "Замерщик": "surveyor_id"}
                ...
                # Для остальных ролей - создаем одну выплату "Полная оплата"
                    ...
                    if self.data.is_multi_user:
                        try:
                            ...
                            calculated_amount = self.data.calculate_payment_amount(contract_id, employee_id, role_name)
                    else:
                        calculated_amount = self.data.calculate_payment_amount(contract_id, employee_id, role_name)
    ...
    def reassign_executor_from_dialog(self, executor_type):
        ...
        if self.card_data is None:
            CustomMessageBox(self, "Ошибка", "Данные карточки не загружены. Пожалуйста, закройте диалог и попробуйте снова.", "error").exec_()
            return
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            CustomMessageBox(self, "Успех", "Исполнитель переназначен! Данные обновлены.", "success").exec_()
    ...
    def reset_card(self):
        ...
        try:
            if self.data.is_multi_user and self.api_client:
                result = self.api_client._request("POST", f"{self.api_client.base_url}/api/v1/crm/{card_id}/reset")
        ...
        try:
            ...
            if not result:
                CustomMessageBox(
                    self,
                    "Диагностика",
                    "Карточка в порядке — восстановление не требуется.\n\nЕсли карточка ещё не начата (стадия не назначена),\nдиагностика станет доступна после назначения исполнителя.",
                    "info",
                ).exec_()
                return
            ...
            changed = old_status != new_status or old_substep != new_substep or old_substage != new_substage
            ...
            if changed:
                ...
                CustomMessageBox(self, "Карточка восстановлена", f'Карточка "{address}" ({column}) восстановлена.\n\nИзменения:\n{details_text}', "success").exec_()
                ...
                CustomMessageBox(self, "Диагностика", f'Карточка "{address}" ({column}) в порядке.\n\nСтатус: {new_status}\nПодэтап: {new_substep}\nГруппа: {new_substage}', "info").exec_()
    ...
    def delete_order(self):
        ...
        if reply == QDialog.Accepted:
            try:
                ...
                CustomMessageBox(self, "Успех", "Заказ успешно удален из системы", "success").exec_()
    ...
    def _update_chat_buttons_state(self):
        ...
        has_chat = self._messenger_chat_data is not None and self._messenger_chat_data.get("chat", {}).get("is_active", False)
        is_online = self.data.is_multi_user

        # Кнопки приглашения — активны если сотрудник назначен и онлайн
        telegram_member_ids = set()
        if has_chat and self._messenger_chat_data:
            for m in self._messenger_chat_data.get("members", []):
                if m.get("member_type") == "employee":
                    telegram_member_ids.add(m.get("member_id"))
        ...
        _can_invite_pos = (self.employee or {}).get("position", "") in ("Руководитель студии", "Старший менеджер проектов", "Менеджер")
        for btn_name, combo_name in invite_btn_combo_map.items():
            btn = getattr(self, btn_name, None)
            combo = getattr(self, combo_name, None)
            if btn and combo:
                emp_id = combo.currentData()
                # Кнопка активна если сотрудник назначен, онлайн и есть право приглашать
                btn.setEnabled(is_online and bool(emp_id) and _can_invite_pos)
                if not emp_id:
                    btn.setToolTip("Сотрудник не назначен")
                elif emp_id in telegram_member_ids:
                    btn.setToolTip("Пригласить в чат (в Telegram уже добавлен)")
                else:
                    btn.setToolTip("Пригласить во внутренний чат / Telegram")
    ...
    def _on_open_chat(self):
        ...
        if invite_link:
            ...
            CustomMessageBox(self, "Ошибка", "Ссылка на чат не найдена. Попробуйте пересоздать чат.", "error").exec_()
    ...
    def _on_delete_chat(self):
        ...
        reply = CustomQuestionBox(self, "Удаление чата", "Внимание! Все сообщения в чате будут удалены безвозвратно.\n\nВы уверены, что хотите удалить чат?").exec_()
    ...
    def _make_invite_btn(self, role_name: str) -> QPushButton:
        """Создать кнопку приглашения сотрудника в чат (внутренний или Telegram)."""
        btn = QPushButton()
        btn.setFixedSize(24, 24)
        btn.setToolTip(f"Пригласить во внутренний чат / Telegram ({role_name})")
        btn.setEnabled(False)
        btn.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        icon = IconLoader.load("message-circle", size=13)
    ...
    # ====== Диалоги внутренних чатов карточки ======

    def _open_card_employee_chat(self):
        """Открыть окно чата сотрудников по данной карточке."""
        self._open_card_chat_dialog("employee")

    def _open_card_client_chat(self):
        """Открыть окно чата с клиентом по данной карточке."""
        self._open_card_chat_dialog("client")

    def _open_card_chat_dialog(self, chat_type: str):
        from PyQt5.QtCore import Qt as _Qt
        from PyQt5.QtWidgets import QDialog, QFrame, QVBoxLayout

        from ui.card_chat_widget import CardChatWidget
        from ui.custom_title_bar import CustomTitleBar
        ...
        title = "Чат сотрудников" if chat_type == "employee" else "Чат с клиентом"
        attr = f"_card_chat_dlg_{chat_type}"
        ...
        title_bar = CustomTitleBar(dlg, title, simple_mode=True)
        title_bar.setStyleSheet("""
            CustomTitleBar {
        ...
        widget = CardChatWidget(
            contract_id=crm_card_id,
            chat_type=chat_type,
            employee=self.employee,
            api_client=self.api_client,
            parent=dlg,
        ...
        dlg.destroyed.connect(lambda: setattr(self, attr, None))
        setattr(self, attr, dlg)
        dlg.exec_()

    # ====== Двойная функция приглашения в чат ======

    def _invite_employee_show_menu(self, combo: QComboBox, role: str, btn: QPushButton):
        """Показать меню выбора: внутренний чат или Telegram."""
        ...
        menu = QMenu(self)
        menu.setStyleSheet("""
            QMenu { background: #fff; border: 1px solid #E0E0E0; border-radius: 4px; padding: 4px 0; }
            QMenu::item { padding: 6px 16px; font-size: 12px; }
            QMenu::item:selected { background: #E3F2FD; }
        """)

        act_internal = menu.addAction("Во внутренний чат (сотрудников)")
        has_telegram = bool(getattr(self, "_messenger_chat_data", None) and self._messenger_chat_data.get("chat", {}).get("is_active"))
        act_telegram = None
        if has_telegram:
            menu.addSeparator()
            act_telegram = menu.addAction("В Telegram-чат")

        chosen = menu.exec_(btn.mapToGlobal(btn.rect().bottomLeft()))
        if chosen == act_internal:
            self._invite_employee_to_internal_chat(combo, role)
        elif act_telegram and chosen == act_telegram:
            self._invite_employee_to_chat(combo, role)

    def _invite_employee_to_internal_chat(self, combo: QComboBox, role: str):
        """Добавить сотрудника участником внутреннего чата сотрудников для данной карточки."""
        from ui.custom_message_box import CustomMessageBox
        ...
        def _worker():
            ...
            try:
                ...
                if not active:
                    ...
                    ok = self.api_client.add_chat_member(active["id"], emp_id)
                    if ok:
                        msg = f"{combo.currentText()} добавлен во внутренний чат сотрудников"
                    else:
                        error = "Не удалось добавить участника"
            except Exception as e:
                err_str = str(e)
                error = err_str.split("): ", 1)[1] if "): " in err_str else err_str
            self._chat_action_finished.emit(msg, error)
    ...
    def create_payments_tab(self):
        ...
        group_info = [("management", "Руководство проекта"), ("support", "Поддержка проекта"), ("executors", "Исполнители")]
        ...
        for group_key, group_title in group_info:
            ...
            table.setHorizontalHeaderLabels(["Должность", "ФИО", "Стадия", "Тип", "Выплата", "Аванс", "Доплата", "Месяц", "Действия"])
        ...
        if has_reassigned:
            warning_label = QLabel("<b>ВНИМАНИЕ!</b> Обнаружено переназначение сотрудников (строки выделены желтым).")
    ...
    def adjust_payment_amount(self, payment_id):
        ...
        """)
        save_btn.clicked.connect(lambda: self.save_manual_amount(payment_id, amount_spin.value(), month_combo.currentIndex() + 1, int(year_combo.currentText()), dialog))
    ...
    def save_manual_amount(self, payment_id, amount, month, year, dialog):
        ...
        if not updated:
            try:
                self.data.update_payment(payment_id, {"manual_amount": amount, "final_amount": amount, "is_manual": True, "report_month": report_month, "reassigned": False})
    ...
    def delete_payment(self, payment_id, role, employee_name):
        ...
        # Подтверждение удаления
        reply = CustomQuestionBox(self, "Подтверждение удаления", f"Вы уверены, что хотите удалить оплату?\n\nДолжность: {role}\nФИО: {employee_name}\n\nЭто действие нельзя отменить!").exec_()
        ...
        if reply == QDialog.Accepted:
            try:
                ...
                CustomMessageBox(self, "Успех", f"Оплата успешно удалена:\n{role} - {employee_name}", "success").exec_()
                ...
                CustomMessageBox(self, "Ошибка", f"Не удалось удалить оплату:\n{str(e)}", "error").exec_()
        ...
        if self._timeline_tab_index >= 0:
            try:
                ...
                self.timeline_widget = ProjectTimelineWidget(
                    card_data=self.card_data, data=self.data, db=self.db, api_client=self.api_client, employee=self.employee, parent=self, readonly=_is_archive
                )
    ...
    def _init_card_chat_tabs(self):
        """Чаты теперь открываются кнопками внизу, не вкладками."""
        pass
    ...
    def _update_project_data_file_labels(self):
        ...
        if tech_task_link and hasattr(self, "project_data_tz_file_label"):
            self.project_data_tz_file_label.setText(f'<a href="{tech_task_link}" title="Открыть папку с тех. заданием">Открыть папку с ТЗ</a>')
        ...
        if measurement_link and hasattr(self, "project_data_survey_file_label"):
            self.project_data_survey_file_label.setText(f'<a href="{measurement_link}" title="Открыть папку с замером">Открыть папку с замером</a>')
            # Кнопка загрузки всегда активна — можно подгружать ещё файлы
        ...
        if ref_path and hasattr(self, "project_data_references_label"):
            self.project_data_references_label.setText(f'<a href="{ref_path}">Открыть папку с референсами</a>')

        # Фотофиксация
        photo_path = contract.get("photo_documentation_yandex_path")
        if photo_path and hasattr(self, "project_data_photo_doc_label"):
            self.project_data_photo_doc_label.setText(f'<a href="{photo_path}">Открыть папку с фотофиксацией</a>')
    ...
    def delete_stage_file(self, file_id, stage):
        ...
        if self.data.is_online and self.employee:
            ...
            if not is_manager:
                try:
                    resp = self.data.api_client._request("GET", f"{self.data.api_client.base_url}/api/files/{file_id}", mark_offline=False)
                    file_check = self.data.api_client._handle_response(resp)
                    if file_check and file_check.get("uploaded_by") != self.employee.get("id"):
                        from ui.custom_message_box import CustomMessageBox

                        CustomMessageBox(self, "Нет доступа", "Вы можете удалять только загруженные вами файлы.", "warning").exec_()
                        return
                except Exception:
                    pass
        ...
        if self.data.is_online:
            try:
                ...
                response = self.data.api_client._request("GET", f"{self.data.api_client.base_url}/api/files/{file_id}", mark_offline=False)
            ...
            try:
                ...
                if not cache_path:
                    ...
                    if contract_id_for_cache and file_name:
                        cache_path = PreviewGenerator.get_cache_path(contract_id_for_cache, stage, file_name) if hasattr(PreviewGenerator, "get_cache_path") else None
    ...
    def upload_stage_files_with_variation(self, stage, variation):
        ...
        def upload_thread():
            try:
                ...
                uploaded_files = yd.upload_stage_files(file_paths, contract_folder, stage, variation=variation, progress_callback=update_progress)
    ...
    def delete_variation_folder(self, stage, variation):
        ...
        reply = CustomQuestionBox(self, "Подтверждение", f"Вы уверены, что хотите удалить Вариацию {variation}?\nВсе файлы этой вариации будут удалены.").exec_()
        ...
        try:
            ...
            if contract_folder:
                variation_folder = yd.get_stage_folder_path(contract_folder, stage, variation=variation)
                if variation_folder:
        ...
        if self.employee and len(variation_files) > 0:
            ...
            # Определяем название стадии для более понятного описания с учетом типа проекта
            stage_names = {"stage2_concept": "Стадия 2 (Концепция)", "stage2_3d": "3 стадия - 3D визуализация (дополнительная)" if is_template else "Стадия 2 (3D)"}
            stage_name = stage_names.get(stage, stage)
    ...
    def _load_all_stage_files_batch(self):
        ...
        stage_files = [f for f in all_files if f.get("stage") in ("stage1", "stage2_concept", "stage2_3d", "stage3")]
        self._stage_files_from_api = len(stage_files) > 0
        print(f"[BATCH] Загружено {len(all_files)} файлов ({len(stage_files)} стадийных), скан ЯД {'пропустим' if self._stage_files_from_api else 'нужен'}")
    ...
    def reload_stage_files(self, stage):
        ...
        if stage == "stage1" and hasattr(self, "stage1_list"):
            ...
            for file_data in files or []:
                ...
                variations[variation].append(file_data)
            for variation, variation_files in variations.items():
                self.stage1_list.load_files_for_variation(variation, variation_files, self.load_preview_for_file)
            ...
            for variation, variation_files in variations.items():
                self.stage2_concept_gallery.load_files_for_variation(variation, variation_files, self.load_preview_for_file)
        elif stage == "stage2_3d" and hasattr(self, "stage2_3d_gallery"):
            # Группируем файлы по вариациям
            variations = {}
            for file_data in files:
            ...
            # Загружаем каждую вариацию
            for variation, variation_files in variations.items():
                self.stage2_3d_gallery.load_files_for_variation(variation, variation_files, self.load_preview_for_file)
            ...
            for file_data in files or []:
                ...
                variations[variation].append(file_data)
            for variation, variation_files in variations.items():
                self.stage3_list.load_files_for_variation(variation, variation_files, self.load_preview_for_file)
    ...
    def _start_background_preview_loading(self):
        ...
        self._preview_loader_thread = PreviewLoaderThread(files_to_load=files_to_load, callback=self._on_preview_loaded_from_thread, yandex_token=YANDEX_DISK_TOKEN)
