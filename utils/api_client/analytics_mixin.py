...
class AnalyticsMixin:
    ...
    def create_survey(self, contract_id: int, project_type: str) -> Dict[str, Any]:
        ...
        response = self._request(
            'POST',
            f"{self.base_url}/api/v1/surveys/create",
    ...
    def get_surveys_by_contract(self, contract_id: int,
        ...
        """GET /api/v1/surveys/contract/{contract_id}"""
        params = {}
        if project_type:
            params['project_type'] = project_type
        response = self._request(
            'GET',
            f"{self.base_url}/api/v1/surveys/contract/{contract_id}",
            params=params
        )
        return self._handle_response(response)

    def resend_survey(self, survey_id: int) -> Dict[str, Any]:
        """POST /api/v1/surveys/{survey_id}/resend"""
        response = self._request(
            'POST',
            f"{self.base_url}/api/v1/surveys/{survey_id}/resend"
        )
        return self._handle_response(response)
    ...
    def get_survey_stats(self, project_type: str = None) -> Dict[str, Any]:
        ...
        response = self._request(
            'GET',
            f"{self.base_url}/api/v1/surveys/stats",
            params=params
        )
        return self._handle_response(response)
