...
`GH_TOKEN` сохранён как постоянная переменная окружения Windows (`setx`).
Это позволяет `gh` CLI работать без popup-окон авторизации.

- **Аккаунт:** FESTIVALCOLOR
- **Scopes:** gist, repo, workflow
- **Источник токена:** GitHub OAuth (prefix `gho_`)

**Обновление токена** (если протух или 401 ошибка):
1. Открыть https://github.com/settings/tokens → Generate new token (classic)
2. Scopes: `repo`, `workflow`, `gist`, `read:org`
3. Выполнить: `setx GH_TOKEN "ghp_новый_токен"` — применится в новых сессиях
4. Для текущей сессии: `export GH_TOKEN="ghp_новый_токен"`
