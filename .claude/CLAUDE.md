...
**ОБЯЗАТЕЛЬНО** перед завершением работы (когда todo-список выполнен) — записать состояние задачи:
```python
import json
state = {"topic": "краткое описание задачи", "todos": [{"content": "...", "status": "completed"}, ...]}
with open(".claude/hooks/task_state.json", "w", encoding="utf-8") as f:
    json.dump(state, f, ensure_ascii=False, indent=2)
```
Hook `Stop` автоматически отправит уведомление в Telegram с темой и todo-списком.
