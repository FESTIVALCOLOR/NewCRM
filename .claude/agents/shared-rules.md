...
## CLAUDE.md first — навигация без лишних tool calls

**ОБЯЗАТЕЛЬНО:** Перед любым Grep/Glob/Read для поиска файлов → сначала проверь `.claude/CLAUDE.md`.
Карта проекта уже содержит ответы. Сокращает tool calls с 10–40 до 1–2.

### Что где искать в CLAUDE.md

| Что нужно | Раздел |
|-----------|--------|
| Какой файл отвечает за X | **Карта ключевых файлов** |
| Метод DataAccess для сущности | **Атрибуты: utils/data_access.py** → группа по домену |
| HTTP endpoint | **Атрибуты: server/routers/** → нужный роутер |
| Значение статуса / типа / колонки | **Бизнес-словарь** |
| Ключ права (`require_permission(...)`) | **Полный список ключей прав** |
| Pinia store мобильного экрана | **Mobile PWA — Pinia Stores** |
| API метод мобильного компонента | **Mobile PWA — API клиент** |
| SQLAlchemy модель (таблица, поля) | **Модели базы данных** |
| Pydantic схема (Create/Update/Response) | **Pydantic схемы** |
| Vue composable | **Mobile PWA — Composables** |

### Когда Grep всё же нужен

- **Нужен:** найти конкретную строку кода, трассировать вызовы символа, проверить все вхождения
- **Не нужен:** найти "какой файл делает X" или "какой метод вызвать" — это уже в CLAUDE.md

### Пример: до и после

```
ДО  (12+ tool calls): Glob("ui/**/*.py") → Read(crm_tab.py) → Grep("workflow") → ...
ПОСЛЕ (1-2 calls):    CLAUDE.md["CRM workflow"] → workflow_submit(id) в data_access.py ✓
                      CLAUDE.md["crm_router"]   → POST /api/crm/cards/{id}/workflow/submit ✓
```

---

## ОБЯЗАТЕЛЬНЫЙ PRE-FLIGHT ПЕРЕД ПЕРВЫМ Edit/Write

**СТОП.** Перед вызовом первого `Edit` или `Write` — выполнить этот протокол.

### Шаги (не пропускать ни один)

```
1. Прочитать запрос ЦЕЛИКОМ ещё раз
2. Ответить на вопрос: «Затрагивает ли это несколько слоёв/файлов?»
3. Grep по изменяемому символу/строке/паттерну — найти ВСЕ вхождения
4. Составить список файлов для изменения
5. Только после п.1–4 → первый Edit
```

### Матрица: что где искать при изменении

| Изменяю | Ещё нужно проверить |
|---------|---------------------|
| `server/routers/` | `utils/api_client/` → `utils/data_access.py` → `ui/` → `mobile/src/` → `tests/e2e/` |
| `utils/data_access.py` | все `ui/*.py` где вызывается метод + `tests/` |
| `ui/*.py` | `mobile/src/pages/` (аналогичный экран?) + `tests/ui/` |
| `mobile/src/` | `mobile/src/services/api.js` + `mobile/src/stores/` |
| `server/database.py` (модель) | `server/schemas.py` + `server/routers/` + `database/migrations.py` |
| `server/schemas.py` (поле) | все роутеры где схема используется + `utils/api_client/` |
| документ/MD файл | все разделы того же файла + связанные docs/ файлы |
| константу / enum значение | Grep по строке по всему проекту |

### Автономный режим ≠ торопливый режим

Правило «не спрашивать подтверждения» = делать без пауз.
**НЕ** = пропускать полный поиск. Поиск — часть задачи, не опциональная проверка.

Частичное исправление (только в одном слое из трёх) **хуже**, чем никакого —
оно создаёт противоречия внутри кодовой базы, которые труднее найти.
