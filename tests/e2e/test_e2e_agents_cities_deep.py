...
def _find_agent_by_name(api_base, headers, name, include_deleted=False):
    ...
    resp = api_get(api_base, "/api/agents", headers,
...
def _cleanup_agent(api_base, headers, name):
    ...
    if agent:
        api_delete(api_base, f"/api/agents/{agent['id']}", headers)
...
def _cleanup_city(api_base, headers, name):
    ...
    if city:
        api_delete(api_base, f"/api/cities/{city['id']}", headers)
...
class TestAgentsDeep:
    ...
    def test_create_and_get_agent(self, api_base, admin_headers):
        """POST /api/agents -> GET /api/agents/{id} — создание и получение."""
        name = f"{TEST_PREFIX}АГЕНТ_DEEP_{int(time.time()) % 100000}"
        _cleanup_agent(api_base, admin_headers, name)

        # Создаём
        resp = api_post(api_base, "/api/agents", admin_headers,
        ...
        # Получаем по ID
        resp2 = api_get(api_base, f"/api/agents/{agent_id}", admin_headers)
    ...
    def test_duplicate_agent_returns_400(self, api_base, admin_headers):
        """POST /api/agents дважды с одинаковым именем -> 400 на втором запросе."""
        name = f"{TEST_PREFIX}ДУБЛИКАТ_DEEP_{int(time.time()) % 100000}"
        _cleanup_agent(api_base, admin_headers, name)

        # Первый — создаём
        resp1 = api_post(api_base, "/api/agents", admin_headers,
                         json={"name": name, "color": "#AABB00"})
        assert resp1.status_code == 200

        # Второй — дубликат
        resp2 = api_post(api_base, "/api/agents", admin_headers,
        ...
        if agent:
            api_delete(api_base, f"/api/agents/{agent['id']}", admin_headers)

    def test_soft_delete_agent(self, api_base, admin_headers):
        """DELETE /api/agents/{id} — мягкое удаление, агент исчезает из активного списка."""
    ...
    def test_include_deleted_shows_deleted(self, api_base, admin_headers):
        """GET /api/agents?include_deleted=true — удалённый агент виден."""
    ...
    def test_update_agent_color(self, api_base, admin_headers):
        """PATCH /api/agents/{name}/color — обновление цвета."""
        ...
        # Обновляем цвет
        patch_resp = api_patch(api_base, f"/api/agents/{name}/color", admin_headers,
                               json={"color": "#FFFFFF"})
    ...
    def test_get_nonexistent_agent_404(self, api_base, admin_headers):
        """GET /api/agents/999999 — несуществующий агент -> 404."""
        resp = api_get(api_base, "/api/agents/999999", admin_headers)
        assert resp.status_code == 404

    def test_delete_nonexistent_agent_404(self, api_base, admin_headers):
        """DELETE /api/agents/999999 — несуществующий агент -> 404."""
        resp = api_delete(api_base, "/api/agents/999999", admin_headers)
        assert resp.status_code == 404

    def test_agents_require_auth(self, api_base):
        """GET /api/agents без авторизации -> 401."""
...
class TestCitiesDeep:
    ...
    def test_create_and_list_city(self, api_base, admin_headers):
        """POST /api/cities -> GET /api/cities — город появляется в списке."""
        name = f"{TEST_PREFIX}ГОРОД_DEEP_{int(time.time()) % 100000}"
        _cleanup_city(api_base, admin_headers, name)

        resp = api_post(api_base, "/api/cities", admin_headers,
                        json={"name": name})
        ...
        # Очистка
        api_delete(api_base, f"/api/cities/{city_id}", admin_headers)

    def test_duplicate_city_returns_400(self, api_base, admin_headers):
        """POST /api/cities с дубликатом -> 400."""
        name = f"{TEST_PREFIX}ДУБГОРОД_DEEP_{int(time.time()) % 100000}"
        _cleanup_city(api_base, admin_headers, name)

        # Создаём первый
        resp1 = api_post(api_base, "/api/cities", admin_headers,
                         json={"name": name})
        assert resp1.status_code in (200, 201)

        # Дубликат
        resp2 = api_post(api_base, "/api/cities", admin_headers,
                         json={"name": name})
        ...
        if city:
            api_delete(api_base, f"/api/cities/{city['id']}", admin_headers)

    def test_soft_delete_city(self, api_base, admin_headers):
        """DELETE /api/cities/{id} — мягкое удаление."""
        ...
        # Удаляем
        del_resp = api_delete(api_base, f"/api/cities/{city_id}", admin_headers)
    ...
    def test_restore_deleted_city(self, api_base, admin_headers):
        """POST /api/cities с именем удалённого города — восстановление."""
    ...
    def test_include_deleted_shows_city(self, api_base, admin_headers):
        """GET /api/cities?include_deleted=true — удалённый город виден."""
    ...
    def test_delete_nonexistent_city_404(self, api_base, admin_headers):
        """DELETE /api/cities/999999 — несуществующий город -> 404."""
        resp = api_delete(api_base, "/api/cities/999999", admin_headers)
        assert resp.status_code == 404

    def test_cities_require_auth(self, api_base):
        """GET /api/cities без авторизации -> 401."""
