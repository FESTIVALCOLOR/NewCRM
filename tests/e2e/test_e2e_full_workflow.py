...
class TestFullBusinessWorkflow:
    ...
    def test_03_create_crm_card(self):
        ...
        if resp.status_code == 409:
            ...
            cards_resp = api_get(self._api_base, "/api/crm/cards", self._headers, params={"project_type": "Индивидуальный"})
    ...
    def test_04_set_card_deadline(self):
        ...
        resp = api_patch(self._api_base, f"/api/crm/cards/{self._state['crm_card_id']}", self._headers, json={"deadline": "2026-04-15"})
        assert resp.status_code == 200
    ...
    def test_05_assign_surveyor_and_measurement(self):
        ...
        if surveyor:
            api_patch(self._api_base, f"/api/crm/cards/{self._state['crm_card_id']}", self._headers, json={"surveyor_id": surveyor["id"]})

        resp = api_patch(self._api_base, f"/api/contracts/{self._state['contract_id']}/files", self._headers, json={"measurement_date": "2026-02-10"})
        assert resp.status_code == 200
    ...
    def test_06_move_to_stage1_assign_designer(self):
        ...
        # В ожидании
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/column", self._headers, json={"column_name": "В ожидании"})
        assert resp.status_code == 200

        # Стадия 1
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/column", self._headers, json={"column_name": "Стадия 1: планировочные решения"})
        assert resp.status_code == 200
    ...
    def test_07_designer_submits_work(self):
        ...
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/stage-executor/{stage_encoded}/complete", self._headers, json={"executor_id": designer["id"]})
        assert resp.status_code == 200
    ...
    def test_10_move_to_stage2_assign_draftsman(self):
        ...
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/column", self._headers, json={"column_name": "Стадия 2: концепция дизайна"})
        assert resp.status_code == 200
    ...
    def test_11_complete_all_stages(self):
        ...
        # Стадия 3
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/column", self._headers, json={"column_name": "Стадия 3: рабочие чертежи"})
        assert resp.status_code == 200
    ...
    def test_12_move_to_completed(self):
        ...
        resp = api_patch(self._api_base, f"/api/crm/cards/{card_id}/column", self._headers, json={"column_name": "Выполненный проект"})
        assert resp.status_code == 200
    ...
    def test_14_upload_supervision_file_correct_stage(self):
        ...
        assert sv_file["stage"] == "supervision", f"Файл надзора должен иметь stage='supervision', а не '{sv_file['stage']}'"

    @pytest.mark.order(15)
    def test_15_verify_all_data_exists(self):
        """Шаг 15: Проверка всех созданных данных"""
        # Клиент
        resp = api_get(self._api_base, f"/api/clients/{self._state['client_id']}", self._headers)
        assert resp.status_code == 200

        # Договор
        resp = api_get(self._api_base, f"/api/contracts/{self._state['contract_id']}", self._headers)
        assert resp.status_code == 200

        # CRM карточка
        resp = api_get(self._api_base, f"/api/crm/cards/{self._state['crm_card_id']}", self._headers)
        assert resp.status_code == 200
        assert resp.json()["column_name"] == "Выполненный проект"

        # Карточка надзора
        resp = api_get(self._api_base, f"/api/supervision/cards/{self._state['supervision_card_id']}", self._headers)
        assert resp.status_code == 200

        # Файлы
        resp = api_get(self._api_base, f"/api/files/contract/{self._state['contract_id']}", self._headers)
        assert resp.status_code == 200
        files = resp.json()
    ...
    def test_16_cleanup_all_test_data(self):
        ...
        # Договор — сначала переводим в архивный статус, чтобы разрешить удаление
        con_id = self._state.get("contract_id")
        if con_id:
            api_patch(self._api_base, f"/api/contracts/{con_id}", self._headers, json={"status": "РАСТОРГНУТ"})
