...
class TestPermissionsConsistency:
    ...
    pytestmark = pytest.mark.skipif(
        ...
        reason="requires PyQt5 (ui.permissions_matrix_widget)",
    )
    ...
    def test_access_tab_map_covers_all_access_perms(self):
        ...
        assert access_perms == map_perms, f"Расхождение: в матрице={access_perms - map_perms}, в маппинге={map_perms - access_perms}"
    ...
    def test_all_descriptions_exist(self):
        ...
        from ui.permissions_matrix_widget import PERMISSION_DESCRIPTIONS, PERMISSION_GROUPS

        for group, perms in PERMISSION_GROUPS.items():
            for perm in perms:
                assert perm in PERMISSION_DESCRIPTIONS, f"Право '{perm}' из группы '{group}' не имеет описания"
