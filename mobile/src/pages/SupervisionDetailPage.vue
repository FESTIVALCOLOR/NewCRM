<template>
  <q-page padding>
    ...
    <template v-else-if="card">
      ...
      <q-tab-panels v-model="activeTab" animated class="bg-transparent">
        ...
        <q-tab-panel name="executors" class="q-pa-none">
          ...
          <q-card class="is-card q-mb-md">
            ...
            <q-list dense>
              ...
              <q-item v-if="card.area"><q-item-section avatar><q-icon name="square_foot" color="grey-7" /></q-item-section><q-item-section><q-item-label caption>Площадь</q-item-label><q-item-label>{{ card.area }} м²</q-item-label></q-item-section></q-item>
              <q-item v-if="card.deadline"><q-item-section avatar><q-icon name="flag" color="grey-7" /></q-item-section><q-item-section><q-item-label caption>Дедлайн проекта</q-item-label><q-item-label :style="{ color: dlColor(card.deadline) }">{{ formatDate(card.deadline) }} ({{ daysLeft(card.deadline) }})</q-item-label></q-item-section></q-item>
            </q-list>
          </q-card>
          ...
          <q-card class="is-card q-mb-md">
            <q-card-section class="q-pb-none">
              <div class="text-subtitle2 text-weight-bold" style="color: #333">Команда</div>
            </q-card-section>
          ...
          <q-card class="is-card q-mb-md">
            <q-card-section class="q-pb-none">
              <div class="row items-center justify-between">
                <div class="text-subtitle2 text-weight-bold" style="color: #333">Стадии закупок</div>
              ...
              <q-item v-for="entry in timeline" :key="entry.id" clickable v-ripple @click="editTimelineEntry(entry)">
                ...
                <q-item-section side>
                  <div class="column items-end q-gutter-xs">
                    ...
                    <q-btn v-if="can('supervision.files_upload')" flat round dense size="xs" icon="attach_file" color="blue" @click.stop="uploadStageFile(entry.stage_code)">
              ...
              <q-item v-for="s in defaultStages" :key="s.code" clickable v-ripple @click="initAndEditStage(s)">
                ...
                <q-item-section side>
                  <div class="row q-gutter-xs items-center">
                    ...
                    <q-btn v-if="can('supervision.files_upload')" flat round dense size="xs" icon="attach_file" color="blue" @click.stop="uploadStageFile(s.code)">
        ...
        <q-tab-panel name="payments" class="q-pa-none">
          <q-card class="is-card q-mb-md">
            <q-card-section class="q-pb-none"><div class="text-subtitle2 text-weight-bold" style="color: #333">Оплаты надзора</div></q-card-section>
        ...
        <q-tab-panel name="chat" class="q-pa-none">
          <div v-if="svChatLoading" class="q-pa-md text-center"><q-spinner color="grey-5" size="24px" /></div>
          ...
          <template v-else-if="svChatData">
            <q-card class="is-card q-mb-md">
              <q-card-section>
                ...
                <div class="text-caption" style="color: #888">{{ svChatMembers.length }} участник(ов)</div>
              </q-card-section>
            </q-card>
          ...
          <template v-else>
            <q-card class="is-card q-mb-md">
              <q-card-section class="text-center q-pa-lg">
                ...
                <q-btn unelevated dense no-caps icon="add" label="Создать чат" style="background: #ffd93c; color: #333; font-size: 12px; font-weight: bold; height: 36px; border-radius: 4px; padding: 0 24px" @click="openCreateSvChatDlg" :loading="svChatCreating" />
              </q-card-section>
            </q-card>
          </template>
        </q-tab-panel>
    ...
    <q-dialog v-model="showCreateSvChatDlg" persistent>
      <q-card style="min-width: 320px; border-radius: 10px">
        <q-toolbar style="background: #ffd93c; color: #333">
          ...
          <q-btn flat round dense icon="close" @click="showCreateSvChatDlg = false" />
        </q-toolbar>
        <q-card-section>
          <q-input v-model="newSvChatTitle" label="Название чата" outlined dense class="q-mb-xs" hint="АН-Город-Адрес" />
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Отмена" v-close-popup no-caps />
          <q-btn unelevated label="Создать" style="background: #ffd93c; color: #333; border-radius: 4px" no-caps @click="createSvChat" :loading="svChatCreating" :disable="!newSvChatTitle.trim()" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showStageFileUpload">
      <q-card style="min-width: 320px; border-radius: 10px">
        <q-toolbar style="background: #5DADE2; color: white">
          <q-toolbar-title class="text-weight-bold" style="font-size: 14px">Загрузить файлы — {{ uploadStageLabel }}</q-toolbar-title>
          <q-btn flat round dense icon="close" color="white" v-close-popup />
        </q-toolbar>
        <q-card-section>
          <q-file
        ...
        <q-card-actions align="right">
          <q-btn flat label="Отмена" v-close-popup no-caps />
          <q-btn unelevated label="Загрузить" no-caps style="background: #5DADE2; color: white; border-radius: 4px" @click="doUploadStageFiles" :loading="stageUploading" :disable="!stageUploadFiles?.length" />
        </q-card-actions>
      </q-card>
    </q-dialog>
